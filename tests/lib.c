...
struct otc_context *srtest_ctx;

void srtest_setup(void)
{
	int ret;

	ret = otc_init(&srtest_ctx);
...
{
	struct otc_dev_driver **drivers, *driver = NULL;
	int i;
...
void srtest_driver_init(struct otc_context *otc_ctx, struct otc_dev_driver *driver)
{
	...
	ret = otc_driver_init(otc_ctx, driver);
	fail_unless(ret == OTC_OK, "Failed to init '%s' driver: %d.",
		    driver->name, ret);
}
...
void srtest_set_samplerate(struct otc_dev_driver *driver, uint64_t samplerate)
{
	...
	ret = driver->config_set(OTC_CONF_SAMPLERATE, gvar, sdi, NULL);
	g_variant_unref(gvar);

	fail_unless(ret == OTC_OK, "%s: Failed to set OTC_CONF_SAMPLERATE: %d.",
		    driver->name, ret);
}
...
uint64_t srtest_get_samplerate(struct otc_dev_driver *driver)
{
	...
	ret = driver->config_get(OTC_CONF_SAMPLERATE, &gvar, sdi, NULL);
	samplerate = g_variant_get_uint64(gvar);
	g_variant_unref(gvar);

	fail_unless(ret == OTC_OK, "%s: Failed to get OTC_CONF_SAMPLERATE: %d.",
		    driver->name, ret);
			     ...
			     uint64_t samplerate)
{
	...
	srtest_driver_init(otc_ctx, driver);;
...
GArray *srtest_get_enabled_logic_channels(const struct otc_dev_inst *sdi)
{
	...
	for (l = otc_dev_inst_channels_get(sdi); l; l = l->next) {
		ch = l->data;
