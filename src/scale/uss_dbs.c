...
static const struct unit units[] = {
	{"g  ", 0, OTC_UNIT_GRAM, 0, {}},
	{"Kg ", 3, OTC_UNIT_GRAM, 0, {}},
	{"ct ", 0, OTC_UNIT_CARAT, 0, {}},
	{"T  ", 0, OTC_UNIT_TOLA, 0, {}},
	{"TAR", 0, OTC_UNIT_TOLA, 0, {1, 16, 6, 10}}, /* tola, aana, ratti, ratti/10 */
	{"dr ", 0, OTC_UNIT_DRAM, 0, {}},
	/* Ratios from experimentation. No idea what the base unit is, so it's not supported yet. */
	/* 10. 0.0.0PKT == 121.50g == 10. 5.0.0TMR*/
	{"PKT", 0, 0, 0, {1, 12, 8, 10}},
	{"GN ", 0, OTC_UNIT_GRAIN, 0, {}},
	{"TMR", 0, OTC_UNIT_TOLA, 0, {1, 12, 8, 10}}, /* tolā, māshā, rattī, rattī/10 */
	/* My particular model seems to assume 10 cm^2 samples, so it's just centigrams. */
	{"gsm", 0, OTC_UNIT_GRAMMAGE, 0, {}},
	{"tIJ", 0, OTC_UNIT_TAEL, OTC_MQFLAG_TAEL_JAPAN, {}},
	{"mo ", 0, OTC_UNIT_MOMME, 0, {}},
	{"dwt", 0, OTC_UNIT_PENNYWEIGHT, 0, {}},
	{"oz ", 0, OTC_UNIT_OUNCE, 0, {}},
	{"lb ", 0, OTC_UNIT_POUND, 0, {}},
	{"tIT", 0, OTC_UNIT_TAEL, OTC_MQFLAG_TAEL_TAIWAN, {}},
	{"ozt", 0, OTC_UNIT_TROY_OUNCE, 0, {}},
	{"tIH", 0, OTC_UNIT_TAEL, OTC_MQFLAG_TAEL_HONGKONG_TROY, {}},
	{"%  ", 0, OTC_UNIT_PERCENTAGE, 0, {}},
	{"pcs", 0, OTC_UNIT_PIECE, 0, {}},
};
...
static enum otc_error_code parse_decimal(const uint8_t *buf, double *result, int *digits)
{
	uint8_t withSign[12];
	...
	return otc_atod_ascii_digits((const char *)withSign, result, digits);
}

static enum otc_error_code parse_component(const uint8_t **buf, double *result)
{
	char toParse[4];
	...
	return otc_atod_ascii((const char *)toParse, result);
}

static enum otc_error_code parse_rational(const uint8_t *buf, const struct unit *u, double *result, int *digits)
{
	int err, f = 1;
	double n, ret = 0;
...
static enum otc_error_code parse_value(const uint8_t *buf, const struct unit *u, double *result, int *digits)
{
	if (u->ratios[0] == 0) {
...
OTC_PRIV enum otc_error_code otc_uss_dbs_parse(const uint8_t *buf, struct otc_datafeed_analog *analog, double *result)
{
	int ret, i, digits = 0;
	char c;
	const struct unit *u;
