 ...
 * @file
 *
 * libopentracecapture private header file, only to be used internally.
 */
...
#define OTC_RECEIVE_DATA_CALLBACK(f) \
	((otc_receive_data_callback) (void (*)(void)) (f))
 ...
 *   identical). The libopentracecapture source code always flushes RX and TX at
...
#ifdef __APPLE__
#define OTC_DRIVER_LIST_SECTION "__DATA,__otc_drivers"
#else
#define OTC_DRIVER_LIST_SECTION "__otc_drivers"
#endif

#if !defined OTC_DRIVER_LIST_NOREORDER && defined __has_attribute
#if __has_attribute(no_reorder)
#define OTC_DRIVER_LIST_NOREORDER __attribute__((no_reorder))
#endif
#endif
#if !defined OTC_DRIVER_LIST_NOREORDER
#define OTC_DRIVER_LIST_NOREORDER /* EMPTY */
#endif
 ...
 * different devices that require different otc_dev_driver struct definitions.
 *
 * For registering only a single driver see OTC_REGISTER_DEV_DRIVER().
 *
 * Example:
 * @code{c}
 ...
 *     };
 *
 * OTC_REGISTER_DEV_DRIVER_LIST(my_driver_infos,
 *     MY_DRIVER("driver 1"),
 *     MY_DRIVER("driver 2"),
 *     ...
 * );
 ...
 * @param ... Comma separated list of pointers to otc_dev_driver structs.
 */
#define OTC_REGISTER_DEV_DRIVER_LIST(name, ...) \
	static const struct otc_dev_driver *name[] \
		OTC_DRIVER_LIST_NOREORDER \
		__attribute__((section (OTC_DRIVER_LIST_SECTION), used, \
			aligned(sizeof(struct otc_dev_driver *)))) \
		= { \
			__VA_ARGS__ \
		};
 ...
 * otc_dev_driver definition.
 *
 * Example:
 * @code{c}
 * static struct otc_dev_driver driver_info = {
 *     .name = "driver",
 *     ....
 * };
 * OTC_REGISTER_DEV_DRIVER(driver_info);
 * @endcode
 *
 * @param name Identifier name of otc_dev_driver struct to register.
 */
#define OTC_REGISTER_DEV_DRIVER(name) \
	OTC_REGISTER_DEV_DRIVER_LIST(name##_list, &name);

OTC_API void otc_drivers_init(struct otc_context *context);

struct otc_context {
	struct otc_dev_driver **driver_list;
#ifdef HAVE_LIBUSB_1_0
	libusb_context *libusb_ctx;
#endif
	otc_resource_open_callback resource_open_cb;
	otc_resource_close_callback resource_close_cb;
	otc_resource_read_callback resource_read_cb;
	void *resource_cb_data;
};

/** Input module metadata keys. */
enum otc_input_meta_keys {
	/** The input filename, if there is one. */
	OTC_INPUT_META_FILENAME = 0x01,
	/** The input file's size in bytes. */
	OTC_INPUT_META_FILESIZE = 0x02,
	/** The first 128 bytes of the file, provided as a GString. */
	OTC_INPUT_META_HEADER = 0x04,

	/** The module cannot identify a file without this metadata. */
	OTC_INPUT_META_REQUIRED = 0x80,
};

/** Input (file) module struct. */
struct otc_input {
	/**
	 * A pointer to this input module's 'struct otc_input_module'.
	 */
	const struct otc_input_module *module;
	GString *buf;
...
struct otc_input_module {
	 ...
	 *   OTC_INPUT_META_FILESIZE
	 *   OTC_INPUT_META_HEADER
	 *
	 * If the high bit (OTC_INPUT META_REQUIRED) is set, the module cannot
	 ...
	 */
	const struct otc_option *(*options) (void);
	 ...
	 *
	 * @retval OTC_OK This module knows the format.
	 * @retval OTC_ERR_NA There wasn't enough data for this module to
	 *   positively identify the format.
	 * @retval OTC_ERR_DATA This module knows the format, but cannot handle
	 *   it. This means the stream is either corrupt, or indicates a
	 *   feature that the module does not support.
	 * @retval OTC_ERR This module does not know the format.
	 *
	 ...
	 */
	int (*init) (struct otc_input *in, GHashTable *options);
	 ...
	 * @retval other Negative error code.
	 */
	int (*receive) (struct otc_input *in, GString *buf);
	 ...
	 * @retval other Negative error code.
	 */
	void (*cleanup) (struct otc_input *in);
};

/** Output module instance. */
struct otc_output {
	/** A pointer to this output's module. */
	const struct otc_output_module *module;
...
struct otc_output_module {
	 ...
	 *
	 * @param o Pointer to the respective 'struct otc_output'.
	 *
	 * @retval OTC_OK Success
	 * @retval other Negative error code.
	 */
	int (*init) (struct otc_output *o, GHashTable *options);
	 ...
	 * @retval other Negative error code.
	 */
	int (*receive) (const struct otc_output *o,
...
struct otc_transform_module {
	 ...
	 *
	 * @param t Pointer to the respective 'struct otc_transform'.
	 ...
	 * @retval other Negative error code.
	 */
	int (*init) (struct otc_transform *t, GHashTable *options);
	 ...
	 * @retval other Negative error code.
	 */
	int (*receive) (const struct otc_transform *t,
	 ...
	 * @retval other Negative error code.
	 */
	int (*cleanup) (struct otc_transform *t);
};
...
OTC_PRIV int otc_log(int loglevel, const char *format, ...) ATTR_FMT_PRINTF(2, 3);

/* Message logging helpers with subsystem-specific prefix string. */
#define otc_spew(...)	otc_log(OTC_LOG_SPEW, LOG_PREFIX ": " __VA_ARGS__)
#define otc_dbg(...)	otc_log(OTC_LOG_DBG,  LOG_PREFIX ": " __VA_ARGS__)
#define otc_info(...)	otc_log(OTC_LOG_INFO, LOG_PREFIX ": " __VA_ARGS__)
#define otc_warn(...)	otc_log(OTC_LOG_WARN, LOG_PREFIX ": " __VA_ARGS__)
#define otc_err(...)	otc_log(OTC_LOG_ERR,  LOG_PREFIX ": " __VA_ARGS__)

/*--- device.c --------------------------------------------------------------*/

/** Scan options supported by a driver. */
#define OTC_CONF_SCAN_OPTIONS 0x7FFF0000

/** Device options for a particular device. */
#define OTC_CONF_DEVICE_OPTIONS 0x7FFF0001

/** Mask for separating config keys from capabilities. */
#define OTC_CONF_MASK 0x1fffffff

/** Values for the changes argument of otc_dev_driver.config_channel_set. */
enum {
	/** The enabled state of the channel has been changed. */
	OTC_CHANNEL_SET_ENABLED = 1 << 0,
};
...
struct otc_dev_inst {
	...
	/** Device instance status. OTC_ST_NOT_FOUND, etc. */
	int status;
	/** Device instance type. OTC_INST_USB, etc. */
	int inst_type;
	/** Device vendor. */
	char *vendor;
	/** Device model. */
	...
	/** List of channels. */
	GSList *channels;
	/** List of otc_channel_group structs */
...
OTC_PRIV void otc_usb_dev_inst_free_cb(gpointer p); /* Glib wrapper. */
#endif
 ...
 * used internally within the libopentracecapture serial layer's implementation,
...
OTC_PRIV int otc_variant_type_check(uint32_t key, GVariant *data);
...
struct otc_session {
	...
	/** List of struct otc_dev_inst pointers. */
	GSList *devs;
	/** List of struct otc_dev_inst pointers owned by this session. */
	GSList *owned_devs;
	...
	otc_session_stopped_callback stopped_callback;
...
OTC_PRIV struct otc_dev_inst *otc_session_prepare_sdi(const char *filename,
		struct otc_session **session);
...
#define zip_discard(zip) otc_zip_discard(zip)
...
typedef int (*dev_close_callback)(struct otc_dev_inst *sdi);
...
extern OTC_PRIV const uint32_t NO_OPTS[1];
...
OTC_PRIV GVariant *std_gvar_min_max_step_thresholds(const double dmin, const double dmax, const double dstep);
...
OTC_PRIV int otc_resource_close(struct otc_context *ctx,
...
OTC_PRIV gssize otc_resource_read(struct otc_context *ctx,
		...
		G_GNUC_WARN_UNUSED_RESULT;
OTC_PRIV void *otc_resource_load(struct otc_context *ctx, int type,
...
struct soft_trigger_logic {
	const struct otc_dev_inst *sdi;
	const struct otc_trigger *trigger;
	int count;
	int unitsize;
	int cur_stage;
	uint8_t *prev_sample;
...
OTC_PRIV void soft_trigger_logic_free(struct soft_trigger_logic *st);
OTC_PRIV int soft_trigger_logic_check(struct soft_trigger_logic *st, uint8_t *buf,
...
typedef GSList *(*otc_ser_list_append_t)(GSList *devs, const char *name,
		const char *desc);
typedef GSList *(*otc_ser_find_append_t)(GSList *devs, const char *name);
...
OTC_PRIV int serial_read_blocking(struct otc_serial_dev_inst *serial, void *buf,
		size_t count, unsigned int timeout_ms);
OTC_PRIV int serial_read_nonblocking(struct otc_serial_dev_inst *serial, void *buf,
		size_t count);
OTC_PRIV int serial_set_read_chunk_cb(struct otc_serial_dev_inst *serial,
		serial_rx_chunk_callback cb, void *cb_data);
OTC_PRIV int serial_set_params(struct otc_serial_dev_inst *serial, int baudrate,
...
OTC_PRIV int serial_set_paramstr(struct otc_serial_dev_inst *serial,
		const char *paramstr);
OTC_PRIV int serial_readline(struct otc_serial_dev_inst *serial, char **buf,
...
struct ser_lib_functions {
	int (*open)(struct otc_serial_dev_inst *serial, int flags);
	int (*close)(struct otc_serial_dev_inst *serial);
	int (*flush)(struct otc_serial_dev_inst *serial);
	int (*drain)(struct otc_serial_dev_inst *serial);
	int (*write)(struct otc_serial_dev_inst *serial,
			const void *buf, size_t count,
			int nonblocking, unsigned int timeout_ms);
	int (*read)(struct otc_serial_dev_inst *serial,
			void *buf, size_t count,
			int nonblocking, unsigned int timeout_ms);
	int (*set_params)(struct otc_serial_dev_inst *serial,
			int baudrate, int bits, int parity, int stopbits,
			int flowcontrol, int rts, int dtr);
	int (*set_handshake)(struct otc_serial_dev_inst *serial,
			int rts, int dtr);
	int (*setup_source_add)(struct otc_session *session,
			struct otc_serial_dev_inst *serial,
			int events, int timeout,
			otc_receive_data_callback cb, void *cb_data);
	int (*setup_source_remove)(struct otc_session *session,
			struct otc_serial_dev_inst *serial);
	GSList *(*list)(GSList *list, otc_ser_list_append_t append);
	GSList *(*find_usb)(GSList *list, otc_ser_find_append_t append,
			uint16_t vendor_id, uint16_t product_id);
	int (*get_frame_format)(struct otc_serial_dev_inst *serial,
			int *baud, int *bits);
	size_t (*get_rx_avail)(struct otc_serial_dev_inst *serial);
};
extern OTC_PRIV struct ser_lib_functions *ser_lib_funcs_libsp;
OTC_PRIV int ser_name_is_hid(struct otc_serial_dev_inst *serial);
extern OTC_PRIV struct ser_lib_functions *ser_lib_funcs_hid;
OTC_PRIV int ser_name_is_bt(struct otc_serial_dev_inst *serial);
extern OTC_PRIV struct ser_lib_functions *ser_lib_funcs_bt;
OTC_PRIV int ser_name_is_tcpraw(struct otc_serial_dev_inst *serial);
extern OTC_PRIV struct ser_lib_functions *ser_lib_funcs_tcpraw;
...
struct ser_hid_chip_functions {
	...
	int (*set_params)(struct otc_serial_dev_inst *serial,
			int baudrate, int bits, int parity, int stopbits,
			int flowcontrol, int rts, int dtr);
	int (*read_bytes)(struct otc_serial_dev_inst *serial,
			uint8_t *data, int space, unsigned int timeout);
	int (*write_bytes)(struct otc_serial_dev_inst *serial,
			const uint8_t *data, int space);
	int (*flush)(struct otc_serial_dev_inst *serial);
	int (*drain)(struct otc_serial_dev_inst *serial);
};
extern OTC_PRIV struct ser_hid_chip_functions *ser_hid_chip_funcs_bu86x;
extern OTC_PRIV struct ser_hid_chip_functions *ser_hid_chip_funcs_ch9325;
extern OTC_PRIV struct ser_hid_chip_functions *ser_hid_chip_funcs_cp2110;
extern OTC_PRIV struct ser_hid_chip_functions *ser_hid_chip_funcs_victor;
...
struct otc_bt_desc;
typedef void (*otc_bt_scan_cb)(void *cb_data, const char *addr, const char *name);
typedef int (*otc_bt_data_cb)(void *cb_data, uint8_t *data, size_t dlen);
...
OTC_PRIV ssize_t otc_bt_read(struct otc_bt_desc *desc,
...
OTC_PRIV int ezusb_install_firmware(struct otc_context *ctx, libusb_device_handle *hdl,
				   const char *name);
 ...
 *
 * @return OTC_OK on success, OTC_ERR_* error code on failure.
 */
...
#define OTC_CRC16_DEFAULT_INIT 0xffffU
...
extern OTC_PRIV const char *ms2115b_channel_formats[];
...
OTC_PRIV gboolean otc_dtm0660_packet_valid(const uint8_t *buf);
OTC_PRIV int otc_dtm0660_parse(const uint8_t *buf, float *floatval,
			struct otc_datafeed_analog *analog, void *info);
...
OTC_PRIV int otc_brymen_bm52x_parse(const uint8_t *buf, float *floatval,
		struct otc_datafeed_analog *analog, void *info);
...
OTC_PRIV int brymen_bm52x_config_get(void *state, uint32_t key, GVariant **data,
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg);
OTC_PRIV int brymen_bm52x_config_set(void *state, uint32_t key, GVariant *data,
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg);
OTC_PRIV int brymen_bm52x_config_list(void *state, uint32_t key, GVariant **data,
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg);
OTC_PRIV int brymen_bm52x_acquire_start(void *state,
	const struct otc_dev_inst *sdi,
	otc_receive_data_callback *cb, void **cb_data);
...
#endif
OTC_PRIV gboolean brymen_bm85x_packet_valid(void *state,
	const uint8_t *buf, size_t len, size_t *pkt_len);
OTC_PRIV int brymen_bm85x_parse(void *state, const uint8_t *buf, size_t len,
	double *floatval, struct otc_datafeed_analog *analog, void *info);
...
OTC_PRIV int otc_brymen_bm86x_parse(const uint8_t *buf, float *floatval,
		struct otc_datafeed_analog *analog, void *info);
...
OTC_PRIV gboolean otc_vc870_packet_valid(const uint8_t *buf);
OTC_PRIV int otc_vc870_parse(const uint8_t *buf, float *floatval,
		struct otc_datafeed_analog *analog, void *info);
...
OTC_PRIV int es51919_config_get(uint32_t key, GVariant **data,
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg);
OTC_PRIV int es51919_config_set(uint32_t key, GVariant *data,
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg);
OTC_PRIV int es51919_config_list(uint32_t key, GVariant **data,
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg);
OTC_PRIV gboolean es51919_packet_valid(const uint8_t *pkt);
OTC_PRIV int es51919_packet_parse(const uint8_t *pkt, float *floatval,
	struct otc_datafeed_analog *analog, void *info);
...
extern OTC_PRIV const char *vc4080_channel_formats[VC4080_CHANNEL_COUNT];
...
OTC_PRIV int vc4080_packet_parse(const uint8_t *pkt, float *floatval,
	struct otc_datafeed_analog *analog, void *info);
...
extern OTC_PRIV const char *eev121gw_channel_formats[];
...
OTC_PRIV int otc_sw_limits_config_get(const struct otc_sw_limits *limits, uint32_t key,
	GVariant **data);
OTC_PRIV int otc_sw_limits_config_set(struct otc_sw_limits *limits, uint32_t key,
	GVariant *data);
