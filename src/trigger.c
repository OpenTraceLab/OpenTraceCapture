 ...
 * matches) using otc_trigger_free() once it is no longer needed.
 *
...
{
	struct otc_trigger *trig;

	trig = g_malloc0(sizeof(struct otc_trigger));
	if (name)
...
OTC_API struct otc_trigger_stage *otc_trigger_stage_add(struct otc_trigger *trig)
{
	...
	stage = g_malloc0(sizeof(struct otc_trigger_stage));
 ...
 *           either of type OTC_CHANNEL_LOGIC or OTC_CHANNEL_ANALOG.
 * @param trigger_match The type of trigger match. Must be a valid trigger
 *                      type from enum otc_trigger_matches. The trigger type
 *                      must be valid for the respective channel type as well.
 * @param value Trigger value.
 *
 * @retval OTC_OK Success.
 * @retval OTC_ERR_ARG Invalid argument(s) were passed to this functions.
 *
 * @since 0.4.0
 */
		...
		struct otc_channel *ch, int trigger_match, float value)
{
	...
	if (ch->type == OTC_CHANNEL_LOGIC) {
		if (trigger_match != OTC_TRIGGER_ZERO &&
				trigger_match != OTC_TRIGGER_ONE &&
				trigger_match != OTC_TRIGGER_RISING &&
				trigger_match != OTC_TRIGGER_FALLING &&
				trigger_match != OTC_TRIGGER_EDGE) {
			otc_err("Invalid trigger match for a logic channel.");
			return OTC_ERR_ARG;
		}
	} else if (ch->type == OTC_CHANNEL_ANALOG) {
		if (trigger_match != OTC_TRIGGER_RISING &&
				trigger_match != OTC_TRIGGER_FALLING &&
				trigger_match != OTC_TRIGGER_EDGE &&
				trigger_match != OTC_TRIGGER_OVER &&
				trigger_match != OTC_TRIGGER_UNDER) {
			otc_err("Invalid trigger match for an analog channel.");
			return OTC_ERR_ARG;
		}
	} else {
		otc_err("Unsupported channel type: %d.", ch->type);
		return OTC_ERR_ARG;
	}

	match = g_malloc0(sizeof(struct otc_trigger_match));
	match->channel = ch;
