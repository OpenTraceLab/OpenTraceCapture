 ...
 * otc_tp_appa_inst tpai;
 *
 * // request packet
 * otc_tp_appa_packet request;
 *
 * // response packet
 * otc_tp_appa_packet response;
 *
 * // return codes are standard sigrok codes
 * int retr;
 *
 * // initialize APPA transport, provide serial port to use
 * if ((retr = otc_tp_appa_init(&tpai, serial)) < OTC_OK)
 *	return retr;
 *
 ...
 * if ((retr = otc_tp_appa_send_receive(&tpai, &request, &response)) < OTC_OK)
 *	return retr;
 *
 * // check if a response was received
 * if (retr > FALSE)
 *	otc_err("Response command was received, command: %d, first byte: %d",
 *		response.command, response.data[0]);
 * else
 *	otc_err("No response received!");
 *
 * @endcode
 *
 */
...
#define OTC_TP_APPA_START_WORD 0x5555
#define OTC_TP_APPA_START_BYTE 0x55
#define OTC_TP_APPA_HEADER_SIZE 4
#define OTC_TP_APPA_RECEIVE_TIMEOUT 500
#define OTC_TP_APPA_PACKET_TIMING 50

#define OTC_TP_APPA_MAX_PAYLOAD_SIZE \
	(OTC_TP_APPA_MAX_DATA_SIZE + OTC_TP_APPA_HEADER_SIZE)

static uint8_t otc_tp_appa_checksum(const uint8_t *arg_data, uint8_t arg_size,
	uint8_t *arg_out_checksum);
static int otc_tp_appa_buffer_reset(struct otc_tp_appa_inst *arg_tpai);
	...
	struct otc_serial_dev_inst *arg_serial)
{
	...
	if (otc_tp_appa_buffer_reset(arg_tpai) < OTC_OK)
		return OTC_ERR_BUG;
	...
	const struct otc_tp_appa_packet *arg_s_packet, gboolean arg_is_blocking)
{
	int retr;
	int dcount;
	uint8_t buf[OTC_TP_APPA_MAX_PACKET_SIZE];
	uint8_t *wrptr;
	uint8_t checksum;
	...
	if (arg_s_packet->length > OTC_TP_APPA_MAX_PAYLOAD_SIZE)
		return OTC_ERR_DATA;

	wrptr = &buf[0];

	write_u16le_inc(&wrptr, OTC_TP_APPA_START_WORD);
	...
	if ((retr = otc_tp_appa_checksum(buf, arg_s_packet->length
		+ OTC_TP_APPA_HEADER_SIZE, &checksum)) < OTC_OK)
		return retr;
	write_u8_inc(&wrptr, checksum);

	dcount = arg_s_packet->length + OTC_TP_APPA_HEADER_SIZE + 1;
	if (arg_is_blocking) {
	...
	struct otc_tp_appa_packet *arg_r_packet, gboolean arg_is_blocking)
{
	...
	int xloop;
	int retr;
	uint8_t buf[OTC_TP_APPA_MAX_PACKET_SIZE * 3];
	uint8_t checksum;
	...
	for (xloop = 0; xloop < len; xloop++) {
		...
		if (arg_tpai->buffer_size < 1) {
			if (buf[xloop] != OTC_TP_APPA_START_BYTE) {
				continue;
			}
		} else if (arg_tpai->buffer_size < 2) {
			if (buf[xloop] != OTC_TP_APPA_START_BYTE) {
				otc_tp_appa_buffer_reset(arg_tpai);
				continue;
			}
		} else if (arg_tpai->buffer_size < 4) {
			if (buf[xloop] > OTC_TP_APPA_MAX_DATA_SIZE) {
				otc_tp_appa_buffer_reset(arg_tpai);
				continue;
			}
		}
		...
		if (arg_tpai->buffer_size > 4) {
			...
			if (arg_tpai->buffer[3]
				+ OTC_TP_APPA_HEADER_SIZE
				+ 1
				== arg_tpai->buffer_size) {

				if ((retr = otc_tp_appa_checksum(arg_tpai->buffer,
					arg_tpai->buffer[3]
					+ OTC_TP_APPA_HEADER_SIZE, &checksum))
					< OTC_OK) {
		...
		if (arg_tpai->buffer_size > OTC_TP_APPA_MAX_PACKET_SIZE) {
			otc_tp_appa_buffer_reset(arg_tpai);
			return OTC_ERR_BUG;
		}
	}
	...
	struct otc_tp_appa_packet *arg_r_packet)
{
	...
	if ((retr = otc_tp_appa_send(arg_tpai, arg_s_packet, TRUE)) < TRUE)
		return retr;

	retr = FALSE;

	trycount = OTC_TP_APPA_RECEIVE_TIMEOUT / OTC_TP_APPA_PACKET_TIMING;
	while (trycount-- > 0) {
		retr = otc_tp_appa_receive(arg_tpai, arg_r_packet, TRUE);
 ...
 * @retval OTC_OK if checksum was calculated and stored in arg_out_checksum
 * @retval OTC_ERR_... on error
 */
static uint8_t otc_tp_appa_checksum(const uint8_t *arg_data, uint8_t arg_size,
	uint8_t *arg_out_checksum)
