...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	if (dmm->after_open) {
		...
		if (ret != OTC_OK) {
			otc_err("Activity after port open failed: %d.", ret);
			return NULL;
		}
	}
	...
	if (dropped > 2 * packet_len)
		otc_warn("Packet search dropped a lot of data.");
	...
	dmm->channel_count = 1;
	if (dmm->packet_parse == otc_brymen_bm52x_parse)
		dmm->channel_count = BRYMEN_BM52X_DISPLAY_COUNT;
	if (dmm->packet_parse == otc_brymen_bm86x_parse)
		dmm->channel_count = BRYMEN_BM86X_DISPLAY_COUNT;
	if (dmm->packet_parse == otc_eev121gw_3displays_parse) {
		dmm->channel_count = EEV121GW_DISPLAY_COUNT;
		dmm->channel_formats = eev121gw_channel_formats;
	}
	if (dmm->packet_parse == otc_metex14_4packets_parse)
		dmm->channel_count = 4;
	if (dmm->packet_parse == otc_ms2115b_parse) {
	...
	for (ch_idx = 0; ch_idx < dmm->channel_count; ch_idx++) {
		...
		otc_channel_new(sdi, ch_idx, OTC_CHANNEL_ANALOG, TRUE, ch_name);
	}
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	struct dmm_info *dmm;
	otc_receive_data_callback cb_func;
	void *cb_data;
	int ret;
	struct otc_serial_dev_inst *serial;
...
OTC_REGISTER_DEV_DRIVER_LIST(serial_dmm_drivers,
	/*
	...
	DMM(
		...
		otc_digitech_qm1578_packet_valid, otc_digitech_qm1578_parse,
		NULL
	),
	...
	DMM(
		...
		otc_asycii_packet_valid, otc_asycii_parse, NULL
	),
	/* }}} */
	/* bm25x based meters {{{ */
	DMM(
		...
		otc_brymen_bm25x_packet_valid, otc_brymen_bm25x_parse,
		NULL
	),
	/* }}} */
	/* bm52x based meters {{{ */
	DMM_CONN(
		...
		otc_brymen_bm52x_packet_request,
		otc_brymen_bm52x_packet_valid, otc_brymen_bm52x_parse,
		NULL
	),
	DMM_CONN(
		"brymen-bm82x", brymen_bm52x, "Brymen", "BM82x",
		"hid/bu86x", NULL, BRYMEN_BM52X_PACKET_SIZE, 4000, 500,
		otc_brymen_bm82x_packet_request,
		otc_brymen_bm82x_packet_valid, otc_brymen_bm52x_parse,
		NULL
	),
	/* }}} */
	/* bm85x based meters {{{ */
	...
	DMM_CONN(
		...
		otc_brymen_bm86x_packet_request,
		otc_brymen_bm86x_packet_valid, otc_brymen_bm86x_parse,
		NULL
	),
	/* }}} */
	...
	DMM(
		...
		otc_dtm0660_packet_valid, otc_dtm0660_parse, NULL
	),
	DMM(
	...
	DMM(
		...
		otc_eev121gw_packet_valid, otc_eev121gw_3displays_parse, NULL
	),
	/* }}} */
	...
	DMM(
		...
		otc_es519xx_2400_11b_packet_valid, otc_es519xx_2400_11b_parse,
		NULL
	),
	/*
	...
	DMM(
		...
		otc_fs9922_packet_valid, otc_fs9922_parse, NULL
	),
	DMM(
	...
	DMM(
		...
		otc_m2110_packet_valid, otc_m2110_parse,
		NULL
	),
	/* }}} */
	...
	DMM(
		...
		METEX14_PACKET_SIZE, 0, 0, otc_metex14_packet_request,
		otc_metex14_packet_valid, otc_metex14_parse,
		NULL
	),
	DMM(
	...
	DMM(
		...
		4 * METEX14_PACKET_SIZE, 0, 0, otc_metex14_packet_request,
		otc_metex14_4packets_valid, otc_metex14_4packets_parse,
		NULL
	),
	DMM(
	...
	DMM(
		...
		METEX14_PACKET_SIZE, 150, 20, otc_metex14_packet_request,
		otc_metex14_packet_valid, otc_metex14_parse,
		NULL
	),
	DMM(
	...
	DMM(
		...
		METEX14_PACKET_SIZE, 250, 60, otc_metex14_packet_request,
		otc_metex14_packet_valid, otc_metex14_parse,
		NULL
	),
	/* }}} */
	...
	DMM(
		...
		otc_ms2115b_packet_valid, otc_ms2115b_parse,
		NULL
	),
	/* }}} */
	...
	DMM(
		...
		otc_ms8250d_packet_valid, otc_ms8250d_parse,
		NULL
	),
	/* }}} */
	/* rs9lcd based meters {{{ */
	DMM(
		...
		otc_rs9lcd_packet_valid, otc_rs9lcd_parse,
		NULL
	),
	/* }}} */
	/* ut71x based meters {{{ */
	...
	DMM(
		...
		otc_vc96_packet_valid, otc_vc96_parse,
		NULL
	),
	/* }}} */
	/*
