...
static int siglent_spd3303_update_status(const struct otc_dev_inst *sdi)
{
	...
	ret = otc_scpi_get_string(scpi, "SYST:STAT?", &response);
	if (ret != OTC_OK)
		return ret;
	if (!response)
		return OTC_ERR;
	otc_atol_base(response, &status_register, NULL, 16);
	g_free(response);
	...
	if (devc->priv_status) {
		...
		for (ch = 0; ch < 2; ch++) {
			...
			if (reg != ((old >> ch) & 0x01)) {
				...
				otc_info("regulation change: ch=%d, reg=%s", ch + 1, reg_str);
				/* FIXME: send OTC_CONF_REGULATION meta frame
				   (when API gets support for channel_group) */
			}
			if (en != ((old >> (ch + 4)) & 0x01)) {
				otc_info("mode change: ch=%d, enabled=%d", ch + 1, en);
				/* FIXME: send OTC_CONF_ENABLED meta frame
				   (when API gets support for channel_group) */
			}
		}
		...
		if (mode != ((old >> 2) & 0x03)) {
			...
			otc_session_send_meta(sdi, OTC_CONF_CHANNEL_CONFIG,
