...
static struct otc_dev_driver yokogawa_dlm_driver_info;
...
static struct otc_dev_inst *probe_device(struct otc_scpi_dev_inst *scpi)
{
	...
	if (dlm_model_get(hw_info->model, &model_name, &model_index) != OTC_OK)
		goto fail;
	...
	if (dlm_device_init(sdi, model_index) != OTC_OK)
		goto fail;
...
static int dev_open(struct otc_dev_inst *sdi)
{
	...
	if (dlm_scope_state_query(sdi) != OTC_OK)
		return OTC_ERR;
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_VDIV:
		...
		if (dlm_analog_chan_vdiv_set(sdi->conn, j + 1, float_str) != OTC_OK ||
	...
	case OTC_CONF_COUPLING:
		...
		if (dlm_analog_chan_coupl_set(sdi->conn, j + 1, (*model->coupling_options)[idx]) != OTC_OK ||
				otc_scpi_get_opc(sdi->conn) != OTC_OK)
			return OTC_ERR;
		ret = OTC_OK;
		break;
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	for (l = sdi->channels; l; l = l->next) {
		...
		if (ch->type != OTC_CHANNEL_LOGIC || !digital_added) {
	...
	if (dlm_check_channels(devc->enabled_channels) != OTC_OK) {
	...
	otc_scpi_source_add(sdi->session, scpi, G_IO_IN, 5,
...
OTC_REGISTER_DEV_DRIVER(yokogawa_dlm_driver_info);
