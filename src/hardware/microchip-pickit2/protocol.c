...
static int pickit2_usb_send(const struct otc_dev_inst *sdi, struct pickit2_cmd *cmd)
{
	...
	text = otc_hexdump_new(&cmd->raw[0], cmd->length);
	otc_dbg("USB sent: %s", text->str);
	...
	if (ret < 0) {
		otc_err("USB transmit error: %s.", libusb_error_name(ret));
		return OTC_ERR_IO;
	}
	if (sent != PICKIT2_PACKET_LENGTH) {
		otc_err("USB short send: %d/%d bytes.",
...
static int pickit2_usb_recv(const struct otc_dev_inst *sdi, struct pickit2_cmd *cmd)
{
	struct otc_usb_dev_inst *usb;
	int ret, rcvd;
	GString *text;
	...
	if (ret < 0) {
		if (ret == LIBUSB_ERROR_TIMEOUT)
			otc_dbg("USB receive error: %s.", libusb_error_name(ret));
		else
			otc_err("USB receive error: %s.", libusb_error_name(ret));
		return OTC_ERR_IO;
	}

	text = otc_hexdump_new(&cmd->raw[0], rcvd);
	otc_dbg("USB recv: %s", text->str);
	otc_hexdump_free(text);

	cmd->length = rcvd;
	if (rcvd != PICKIT2_PACKET_LENGTH) {
		otc_err("USB short recv: %d/%d bytes.",
...
static int pickit2_usb_send_recv(const struct otc_dev_inst *sdi,
...
static int pickit2_retrieve_bank(struct otc_dev_inst *sdi,
...
static int pickit2_retrieve_sample_data(struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	uint8_t *rdpos;
	size_t rdlen;
...
/* Send converted sample data to the opentracelab session. */
static int pickit2_submit_logic_data(struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	struct {
	 ...
	 * internal opentracelab format (one sample per byte, at increasing
...
#define handle_nibble(n) do { \
	...
	*conv_buf++ = conv_byte; \
	...
	/* Submit a logic packet to the opentracelab session. */
...
static gboolean pickit2_status_is_cancel(uint16_t status)
{
	...
	otc_dbg("recv: status 0x%04x", status);
...
OTC_PRIV int microchip_pickit2_receive_data(int fd, int revents, void *cb_data)
{
	...
	if (devc->state == STATE_WAIT) {
		...
		if (pickit2_status_is_cancel(status)) {
			otc_info("User cancelled acquisition.");
			otc_dev_acquisition_stop(sdi);
			return TRUE;
		}
		otc_dbg("recv: Data has become available.");
		/* FALLTHROUGH */
	}
