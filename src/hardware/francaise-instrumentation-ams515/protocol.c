	...
	if (revents == 0 /*G_IO_IN*/) {
		...
		if (!devc->overcurrent) {
			res = francaise_instrumentation_ams515_query_str(
				sdi, 'S', answer);
			if (res == OTC_OK && answer[0] >= 'A' &&
			    answer[0] <= 'C')
				devc->selected_channel = answer[0] - 'A';
			otc_dbg("Selected channel %d.", devc->selected_channel);
					  ...
					  gboolean echoed)
{
	...
	for (i = 0; i < cmdlen; i++) {
		...
		if (serial_write_blocking(serial, &cmd[i], 1,
			...
			otc_err("Write error for cmd[%d].", i);
			break;
		}
		...
		if (serial_read_blocking(serial, &c, 1,
			...
			otc_dbg("Unable to read echoed cmd, assuming no echo.");
			echoed = FALSE;
			continue;
		}
		echoed = TRUE;
		if (c != cmd[i]) {
			otc_err("Mismatched echoed cmd: %c != %c", c, cmd[i]);
	...
	for (i = 0; i < ANSWER_MAX - 1; i++) {
		if (serial_read_blocking(serial, &answer[i], 1,
					 SERIAL_READ_TIMEOUT_MS) < 1) {
			otc_err("Unable to read cmd answer.");
			break;
		}
		//otc_spew("send_raw(): answer: 0x%02x '%c'", answer[i], answer[i]);
		// skip CR/LF
		if (answer[i] == '\r' || answer[i] == '\n') {
			if (!echoed) {
				...
				otc_dbg("CR/LF found reply in no-echo mode!");
				devc->resync = TRUE;
				break;
			}
			i--;
	...
	answer[i] = '\0';
	otc_spew("send_raw(): answer: '%s'", answer);
					   ...
					   const char cmd, int *result)
{
	...
	snprintf(command, 4, "%c?\r", cmd);
	otc_dbg("query_int(): sending %s", command);
					   ...
					   const char cmd, char *result)
{
	...
	snprintf(command, 4, "%c?\r", cmd);
	otc_dbg("query_str(): sending %s", command);
					  ...
					  const char cmd, int param)
{
	...
	snprintf(command, 12, "%c%c%.02X\r", cmd, param < 0 ? '-' : '+',
		 abs(param));
	otc_dbg("send_int(): sending %s", command);
					   ...
					   const char cmd, char param)
{
	...
	otc_dbg("send_char(): sending %s", command);
