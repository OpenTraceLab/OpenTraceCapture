...
static struct otc_dev_driver francaise_instrumentation_ams515_driver_info;
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	if (serial_open(serial, SERIAL_RDWR) != OTC_OK) {
		otc_err("Unable open serial port.");
		return NULL;
	}
...
static int dev_open(struct otc_dev_inst *sdi)
{
	...
	if (res != OTC_OK)
		otc_dbg("Failed to disable echo on unit.");
...
OTC_REGISTER_DEV_DRIVER(francaise_instrumentation_ams515_driver_info);
