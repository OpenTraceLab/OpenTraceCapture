...
static void stop_acquisition(const struct otc_dev_inst *sdi);
...
OTC_PRIV void ftdi_la_store_samplerate(const struct otc_dev_inst *sdi, uint64_t requested_rate)
{
	...
	if (requested_rate_millihz != devc->cur_clk.rate_millihz) {
		otc_warn("Chip does not support sample rate %" PRIu64
			"; adjusted to %" PRIu64 ".%03" PRIu64 ".",
			requested_rate, devc->cur_clk.rate_millihz / 1000,
			devc->cur_clk.rate_millihz % 1000);
	} else {
		otc_info("Configured exact sample rate %" PRIu64 ".", requested_rate);
	}
}

static int write_samplerate(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
...
static void send_samples(const struct otc_dev_inst *sdi, unsigned char *buf, size_t len)
{
	struct dev_context *devc = sdi->priv;
	...
	for (size_t i = 0; i < len; i += devc->in_ep_pkt_size) {
		...
		if (pkt_len < NUM_STATUS_BYTES) {
			otc_warn("Received data packet with no modem status prefix!");
			continue;
		}
		...
		if (data_len == 0) {
			otc_info("Received empty data packet");
			continue;
		}
		...
		otc_spew("Sending %zu samples.", data_len);
...
static void LIBUSB_CALL receive_transfer(struct libusb_transfer *transfer)
{
	...
	otc_spew("receive_transfer called");
	...
	if (transfer->status == LIBUSB_TRANSFER_ERROR ||
		...
		otc_err("USB transfer failed: %s.", libusb_error_name(transfer->status));
		stop_acquisition(sdi);
		goto cleanup_transfer;
	}

	otc_spew("Processing completed transfer of length %d.", transfer->actual_length);
	...
	if (!devc->acq_aborted) {
		...
		if (ret != 0) {
			otc_err("USB transfer submission failed: %s.", libusb_error_name(ret));
			stop_acquisition(sdi);
			goto cleanup_transfer;
		}
...
cleanup_transfer:
	...
	if (--devc->active_transfers == 0) {
		...
		otc_info("Freed all transfer allocations.");

		usb_source_remove(sdi->session, sdi->session->ctx);
	}
}

static int alloc_transfers(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
	struct otc_usb_dev_inst *usb = sdi->conn;
	size_t num_xfers;
	...
	otc_dbg("Using %zu USB transfers of size %u.", num_xfers, bytes_per_xfer);
	...
	if (!devc->transfers) {
		otc_err("Failed to allocate USB transfer pointers.");
	...
	for (size_t i = 0; i < num_xfers; i++) {
		...
		if (!buf || !devc->transfers[i]) {
			otc_err("Ran out of memory while allocating transfers.");
			return OTC_ERR_MALLOC;
		}
...
static int handle_event(int fd, int revents, void *cb_data)
{
	...
	if (ret != 0) {
		otc_err("libusb event handling failed: %s.", libusb_error_name(ret));
		stop_acquisition(sdi);
		return FALSE;
	}
...
OTC_PRIV int ftdi_la_start_acquisition(const struct otc_dev_inst *sdi)
{
	...
	if (ret < 0) {
		otc_err("Failed to reset FTDI chip: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	if (ret < 0) {
		otc_err("Failed to put FTDI chip into bitbang mode: %s.",
	...
	if (ret < 0) {
		otc_err("Failed to set FTDI latency timer: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	for (size_t i = 0; i < devc->num_transfers; i++) {
		if (ret == 0) {
			 ...
			 * their callbacks. */
			if (ret != 0) {
				otc_err("USB transfer initial submission failed: %s.",
						libusb_error_name(ret));
				stop_acquisition(sdi);
			}
		}
