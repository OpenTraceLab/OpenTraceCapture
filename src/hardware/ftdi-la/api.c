...
static void scan_device(struct libusb_device *dev, GSList **devices, int iface_idx)
{
	...
	if ((rv = libusb_get_active_config_descriptor(dev, &config)) != 0) {
		otc_warn("Failed to get config descriptor for device: %s.",
	...
	if (config->bNumInterfaces < num_ifaces) {
		otc_err("Found FTDI device with fewer USB interfaces than we "
			"expect for its type. This is a bug in OpenTraceCapture.");
		goto out_free_config;
	}

	otc_dbg("Found a %d-channel FTDI device: %s.", num_ifaces, model);
	...
	for (unsigned int i = 0; i < num_ifaces; i++) {
		...
		if (config->interface[i].num_altsetting <= 0) {
			otc_err("FTDI interface %d has bad num_altsetting %d",
		...
		if (in_ep_idx == -1) {
			otc_err("FTDI interface %d has no bulk IN endpoint", i);
			goto out_free_config;
		}
		...
		sdi->conn = otc_usb_dev_inst_new(libusb_get_bus_number(dev),
				libusb_get_device_address(dev), NULL);

		for (int chan = 0; chan < 8; chan++)
			otc_channel_new(sdi, chan, OTC_CHANNEL_LOGIC, TRUE,
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	if (conn && conn[0]) {
		...
		if (conn_parts[0][0]) {
			conn_has_usb = TRUE;
			conn_devices = otc_usb_find(drvc->otc_ctx->libusb_ctx, conn_parts[0]);
		}

		/* Interface identifier (e.g. A or B; case-insensitive) */
		if (conn_parts[1]) {
			if (strlen(conn_parts[1]) != 1 || !isalpha(conn_parts[1][0])) {
				otc_err("Invalid interface ID: %s.", conn_parts[1]);
			} else {
...
static GVariant *get_samplerates(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
