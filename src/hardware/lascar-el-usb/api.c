...
static int lascar_proc_config(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	struct otc_usb_dev_inst *usb;
	int dummy, ret;
	...
	case LOG_TEMP_RH:
		...
		if (devc->temp_unit != 0 && devc->temp_unit != 1) {
			otc_dbg("invalid temperature unit %d", devc->temp_unit);
			/* Default to Celsius, we're all adults here. */
			devc->temp_unit = 0;
		} else
			otc_dbg("temperature unit is %s", devc->temp_unit
					? "Fahrenheit" : "Celsius");
		break;
	case LOG_CO:
		devc->sample_size = 2;
		...
		otc_dbg("EL-USB-CO calibration high %f low %f", devc->co_high,
				devc->co_low);
		break;
	default:
		ret = OTC_ERR_ARG;
	...
	otc_dbg("device log contains %d samples.", devc->logged_samples);
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	if (lascar_proc_config(sdi) != OTC_OK)
		return OTC_ERR;

	otc_dbg("Starting log retrieval.");
	...
	packet.payload = &meta;
	src = otc_config_new(OTC_CONF_SAMPLE_INTERVAL, g_variant_new_uint64(interval));
	...
	if (xfer_in->user_data != GINT_TO_POINTER(1) ||
		...
		otc_dbg("no response to log transfer request");
		libusb_free_transfer(xfer_in);
		libusb_free_transfer(xfer_out);
		return OTC_ERR;
	}
	if (xfer_in->actual_length != 3 || xfer_in->buffer[0] != 2) {
		otc_dbg("invalid response to log transfer request");
		libusb_free_transfer(xfer_in);
		libusb_free_transfer(xfer_out);
		return OTC_ERR;
	}
...
OTC_PRIV struct otc_dev_driver lascar_el_usb_driver_info = {
	.name = "lascar-el-usb",
...
OTC_REGISTER_DEV_DRIVER(lascar_el_usb_driver_info);
