...
static const uint32_t devopts_cg[] = {
	...
	OTC_CONF_REGULATION | OTC_CONF_GET | OTC_CONF_LIST,
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	if (serial_write_blocking(serial, CMD_MONITOR_STOP,
		...
		otc_dbg("Unable to write while probing for hardware.");
		serial_close(serial);
		return NULL;
	}
	g_usleep(50 * 1000);
	...
	if (ret < 0 || len < 9 || strncmp((const char *)&buf, "version ", 8)) {
		otc_dbg("Unable to probe version number.");
		serial_close(serial);
		return NULL;
	}
	...
	if (version < 1.10) {
		otc_info("Firmware >= 1.10 required (got %1.2f).", version);
		serial_close(serial);
		return NULL;
	}
	...
	ch = otc_channel_new(sdi, 0, OTC_CHANNEL_ANALOG, TRUE, "V");
	cg->channels = g_slist_append(cg->channels, ch);

	ch = otc_channel_new(sdi, 1, OTC_CHANNEL_ANALOG, TRUE, "I");
	cg->channels = g_slist_append(cg->channels, ch);
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	 ...
	 *  - OTC_CONF_ENABLED (state cannot be queried, only set)
	 */
	...
	case OTC_CONF_CURRENT:
		...
		*data = g_variant_new_double(fvalue);
		break;
	case OTC_CONF_CURRENT_LIMIT:
		if (reloadpro_get_current_limit(sdi, &fvalue) == OTC_OK)
			*data = g_variant_new_double(fvalue);
		break;
	...
	case OTC_CONF_UNDER_VOLTAGE_CONDITION:
		if (reloadpro_get_under_voltage_threshold(sdi, &fvalue) == OTC_OK)
...
static int dev_close(struct otc_dev_inst *sdi)
{
	if (serial_write_blocking(sdi->conn, CMD_MONITOR_STOP,
			...
			strlen(CMD_MONITOR_STOP))) < (int)strlen(CMD_MONITOR_STOP)) {
		otc_dbg("Unable to stop monitoring.");
	}
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	if ((ret = serial_write_blocking(serial, CMD_MONITOR,
		...
		otc_err("Unable to send 'monitor' command: %d.", ret);
		return OTC_ERR;
	}
...
static struct otc_dev_driver arachnid_labs_re_load_pro_driver_info = {
...
OTC_REGISTER_DEV_DRIVER(arachnid_labs_re_load_pro_driver_info);
