...
static const struct {
	...
	{ OTC_MQ_VOLTAGE, set_mq_volt },
	{ OTC_MQ_CURRENT, set_mq_amp },
	{ OTC_MQ_RESISTANCE, set_mq_ohm },
};

static int set_mq_volt(struct otc_scpi_dev_inst *scpi, enum otc_mqflag flags)
{
	if ((flags & OTC_MQFLAG_AC) != OTC_MQFLAG_AC &&
		(flags & OTC_MQFLAG_DC) != OTC_MQFLAG_DC)
		return OTC_ERR_NA;

	return otc_scpi_send(scpi, "%s",
		((flags & OTC_MQFLAG_AC) == OTC_MQFLAG_AC) ? "F2" : "F1");
}
...
static int set_mq_amp(struct otc_scpi_dev_inst *scpi, enum otc_mqflag flags)
{
	...
	return otc_scpi_send(scpi, "%s", (flags & OTC_MQFLAG_AC) ? "F6" : "F5");
}

static int set_mq_ohm(struct otc_scpi_dev_inst *scpi, enum otc_mqflag flags)
{
	return otc_scpi_send(scpi, "%s",
		(flags & OTC_MQFLAG_FOUR_WIRE) ? "F4" : "F3");
}
...
OTC_PRIV int hp_3478a_set_range(const struct otc_dev_inst *sdi, int range_exp)
{
	...
	if (range_exp == -99)
		ret = otc_scpi_send(scpi, "RA");
	else
		ret = otc_scpi_send(scpi, "R%i", range_exp);
	if (ret != OTC_OK)
		return ret;
...
OTC_PRIV int hp_3478a_set_digits(const struct otc_dev_inst *sdi, uint8_t digits)
{
	...
	ret = otc_scpi_send(scpi, "N%i", digits-1);
	if (ret != OTC_OK)
		return ret;
...
static int parse_function_byte(struct dev_context *devc, uint8_t function_byte)
{
	...
	if ((function_byte & SB1_FUNCTION_BLOCK) == FUNCTION_VDC) {
		devc->measurement_mq = OTC_MQ_VOLTAGE;
		devc->measurement_mq_flag = OTC_MQFLAG_DC;
		devc->acquisition_mq_flags |= OTC_MQFLAG_DC;
		devc->measurement_unit = OTC_UNIT_VOLT;
		parse_range_vdc(devc, function_byte);
	} else if ((function_byte & SB1_FUNCTION_BLOCK) == FUNCTION_VAC) {
		devc->measurement_mq = OTC_MQ_VOLTAGE;
		devc->measurement_mq_flag = OTC_MQFLAG_AC;
		devc->acquisition_mq_flags |= OTC_MQFLAG_AC | OTC_MQFLAG_RMS;
		devc->measurement_unit = OTC_UNIT_VOLT;
		parse_range_vac(devc, function_byte);
	} else if ((function_byte & SB1_FUNCTION_BLOCK) == FUNCTION_2WR) {
		devc->measurement_mq = OTC_MQ_RESISTANCE;
		devc->measurement_unit = OTC_UNIT_OHM;
		parse_range_ohm(devc, function_byte);
	} else if ((function_byte & SB1_FUNCTION_BLOCK) == FUNCTION_4WR) {
		devc->measurement_mq = OTC_MQ_RESISTANCE;
		devc->measurement_mq_flag = OTC_MQFLAG_FOUR_WIRE;
		devc->acquisition_mq_flags |= OTC_MQFLAG_FOUR_WIRE;
		devc->measurement_unit = OTC_UNIT_OHM;
		parse_range_ohm(devc, function_byte);
	} else if ((function_byte & SB1_FUNCTION_BLOCK) == FUNCTION_ADC) {
		devc->measurement_mq = OTC_MQ_CURRENT;
		devc->measurement_mq_flag = OTC_MQFLAG_DC;
		devc->acquisition_mq_flags |= OTC_MQFLAG_DC;
		devc->measurement_unit = OTC_UNIT_AMPERE;
...
static int parse_status_byte(struct dev_context *devc, uint8_t status_byte)
{
	...
	if ((status_byte & STATUS_AUTO_RANGE) == STATUS_AUTO_RANGE) {
		devc->acquisition_mq_flags |= OTC_MQFLAG_AUTORANGE;
		devc->range_exp = -99;
	} else
		devc->acquisition_mq_flags &= ~OTC_MQFLAG_AUTORANGE;
...
OTC_PRIV int hp_3478a_get_status_bytes(const struct otc_dev_inst *sdi)
{
	...
	ret = otc_scpi_get_string(scpi, "B", &response);
	if (ret != OTC_OK)
		return ret;
...
static void acq_send_measurement(struct otc_dev_inst *sdi)
{
	...
	otc_analog_init(&analog, &encoding, &meaning, &spec, devc->otc_digits);
	...
	encoding.digits = devc->otc_digits;
	...
	spec.spec_digits = devc->otc_digits;
...
OTC_PRIV int hp_3478a_receive_data(int fd, int revents, void *cb_data)
{
	 ...
	 */
	if (otc_scpi_gpib_spoll(scpi, &status_register) != OTC_OK)
		return FALSE;
	if (!(((uint8_t)status_register) & SRQ_BUS_AVAIL))
		return TRUE;

	/* Get a reading from the DMM. */
	if (otc_scpi_get_double(scpi, NULL, &devc->measurement) != OTC_OK)
		return FALSE;
	 ...
	 * status register!
	 */
	if (hp_3478a_get_status_bytes(sdi) != OTC_OK)
		return FALSE;
