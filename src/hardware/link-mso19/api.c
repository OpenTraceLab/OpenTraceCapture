		...
		double trigger_level)
{
	...
	if (trigger_level != devc->dso_trigger_adjusted)
		otc_info("Adjusted dso trigger level to %f", devc->dso_trigger_adjusted);
}
...
static void mso_set_trigger_pos(struct dev_context *devc, double trigger_pos)
{
	...
	if (pos < 0) {
		...
		if (pos < (MSO_NUM_SAMPLES + 10)) {
			...
			otc_warn("Trigger holdoff > -1.011 may be unstable %f", devc->horiz_triggerpos);
		}
		...
		otc_warn("Trigger position < .01 may be unstable %f", devc->horiz_triggerpos);
	}
...
static void mso_set_offset_value(struct dev_context *devc, double offset)
{
	...
	if (limited_value != scaled_value) {
		otc_info("Adjusted dso offset to %f", devc->dso_offset_adjusted);
	}
...
static GSList* scan_handle_port(GSList *devices, struct sp_port *port)
{
	...
	if (mso_parse_serial(serial_num, product, devc) != OTC_OK) {
		otc_err("Invalid serial: %s.", serial_num);
		g_free(devc);
		return devices;
	}
	...
	devc->serial = otc_serial_dev_inst_new(port_name, SERIALCOMM);
	...
	cg->channels = g_slist_append(cg->channels,
			otc_channel_new(sdi, i, OTC_CHANNEL_ANALOG, TRUE, "DSO"));
...
static int dev_open(struct otc_dev_inst *sdi)
{
	...
	if (std_serial_dev_open(sdi) != OTC_OK)
		return OTC_ERR;
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_LIMIT_SAMPLES:
		...
		if (tmp_u64 != MSO_NUM_SAMPLES) {
			otc_err("Only %d samples are supported.", MSO_NUM_SAMPLES);
			return OTC_ERR_ARG;
		}
	...
	case OTC_CONF_HORIZ_TRIGGERPOS:
		...
		if (pos < -10.0 || pos > 1.0) {
			otc_err("Trigger position (%f) should be between -10.0 and 1.0", pos);
			return OTC_ERR_ARG;
		}
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	if (mso_configure_channels(sdi) != OTC_OK) {
...
static struct otc_dev_driver link_mso19_driver_info = {
	.name = "link-mso19",
...
OTC_REGISTER_DEV_DRIVER(link_mso19_driver_info);
