...
static struct otc_dev_driver dcttech_usbrelay_driver_info;

static struct otc_dev_inst *probe_device_common(const char *path,
	...
	const wchar_t *vendor, const wchar_t *product)
{
	...
	while (s > nonws && isdigit((int)s[-1]))
		s--;
	ret = otc_atoul_base(s, &relay_count, &endp, 10);
	if (ret != OTC_OK || !endp || *endp)
		return NULL;
	if (!relay_count)
		return NULL;
	otc_info("Relay count %lu from product string %s.", relay_count, nonws);
	...
	if (otc_log_loglevel_get() >= OTC_LOG_SPEW) {
		...
		otc_spew("Got report bytes: %s, rc %d.", txt->str, ret);
		otc_hexdump_free(txt);
	}
	if (ret < 0) {
		otc_err("Cannot read %s: %ls.", path, hid_error(NULL));
		return NULL;
	}
	if (ret != sizeof(report)) {
		otc_err("Unexpected HID report length %d from %s.", ret, path);
		return NULL;
	}
	...
	for (snr_pos = 0; snr_pos < SERNO_LENGTH; snr_pos++) {
		...
		serno[snr_pos] = c;
		if (c < 0x20 || c > 0x7e) {
			otc_warn("Skipping %s, non-printable serial.", path);
			return NULL;
		}
	}
	curr_state = report[1 + STATE_INDEX];
	otc_info("HID report data: serial number %s, relay state 0x%02x.",
		serno, curr_state);

	/* Optionally filter by serial number. */
	if (want_serno && *want_serno && strcmp(serno, want_serno) != 0) {
		otc_dbg("Serial number does not match user spec. Skipping.");
		return NULL;
	}
...
static struct otc_dev_inst *probe_device_enum(struct hid_device_info *dev,
...
static struct otc_dev_inst *probe_device_conn(const char *path)
{
	char vid_pid[12];
	uint16_t vid, pid;
	const char *s;
	...
	vid = pid = 0;
	s = path;
	ret = otc_atoul_base(s, &num, &endp, 16);
	if (ret == OTC_OK && endp && endp == s + 4 && *endp == '.' && num) {
		vid = num;
		s = ++endp;
	}
	ret = otc_atoul_base(s, &num, &endp, 16);
	if (ret == OTC_OK && endp && endp == s + 4 && *endp == '\0' && num) {
		pid = num;
		s = ++endp;
	}
	if (vid && pid) {
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	if (conn) {
		otc_info("Checking HID path %s.", conn);
		sdi = probe_device_conn(conn);
		if (!sdi)
			otc_warn("Failed to communicate to %s.", conn);
		else
	...
	for (curdev = devs; curdev; curdev = curdev->next) {
		...
		if (!*curdev->manufacturer_string || !*curdev->product_string)
			continue;
		otc_dbg("Checking %04hx:%04hx, vendor %ls, product %ls.",
		...
		otc_info("Checking HID path %s.", curdev->path);
...
OTC_REGISTER_DEV_DRIVER(dcttech_usbrelay_driver_info);
