...
static struct otc_dev_driver devantech_eth008_driver_info;

static struct otc_dev_inst *probe_device_conn(const char *conn)
{
	...
	sdi->priv = devc;
	ser = otc_serial_dev_inst_new(conn, NULL);
	sdi->conn = ser;
	if (!ser)
		goto probe_fail;
	...
	if (!model) {
		otc_err("Unknown model ID 0x%02x (HW %u, FW %u).",
			code, hwver, fwver);
		goto probe_fail;
	}
...
OTC_REGISTER_DEV_DRIVER(devantech_eth008_driver_info);
