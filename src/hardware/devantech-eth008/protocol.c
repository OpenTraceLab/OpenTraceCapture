 ...
 * This opentracelab driver implementation was created based on information in
 ...
 * this opentracelab driver implementation. Binary transmission is preferred
 ...
 * This opentracelab driver also assumes complete reception within a single
 ...
 * opentracelab driver implementation. [This also means that a transport
 ...
 *
 * The opentracelab driver source code was phrased with the addition of more
 ...
 */
static int send_request(struct otc_serial_dev_inst *ser,
	const uint8_t *data, size_t dlen)
{
	...
	if (otc_log_loglevel_get() >= OTC_LOG_SPEW) {
		GString *txt = otc_hexdump_new(data, dlen);
		otc_spew("TX --> %s.", txt->str);
 ...
 */
static int recv_response(struct otc_serial_dev_inst *ser,
	uint8_t *data, size_t dlen)
{
	...
	got = (size_t)ret;
	if (otc_log_loglevel_get() >= OTC_LOG_SPEW) {
		GString *txt = otc_hexdump_new(data, got);
		otc_spew("<-- RX %s.", txt->str);
		otc_hexdump_free(txt);
	}
	if (got != dlen)
		return OTC_ERR_DATA;
...
static int send_then_recv(struct otc_serial_dev_inst *serial,
