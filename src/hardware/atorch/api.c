...
static struct otc_dev_driver atorch_driver_info;
...
static int create_channels_feed_queues(struct otc_dev_inst *sdi,
	struct dev_context *devc)
{
	size_t i;
	struct otc_channel *otc_ch;
	...
	for (i = 0; i < p->channel_count; i++) {
		at_ch = &p->channels[i];
		otc_ch = otc_channel_new(sdi, i, OTC_CHANNEL_ANALOG, TRUE, at_ch->name);
		feed = feed_queue_analog_alloc(sdi, 1, at_ch->digits, otc_ch);
...
static GSList *atorch_scan(struct otc_dev_driver *di,
	const char *conn, const char *serialcomm)
{
	...
	if (atorch_probe(serial, devc) != OTC_OK) {
		otc_err("Failed to find a supported Atorch device.");
		goto err_out_serial;
	}
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	(void)otc_serial_extract_options(options, &serial_device, &serial_options);
...
OTC_REGISTER_DEV_DRIVER(atorch_driver_info);
