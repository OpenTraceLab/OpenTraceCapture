...
static struct otc_dev_driver pslab_driver_info;
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	GSList *device_paths_v5 = otc_serial_find_usb(0x04D8, 0x00DF);

	GSList *device_paths_v6 = otc_serial_find_usb(0x10C4, 0xEA60);
	...
	for (l = device_paths; l; l = l->next) {
		...
		serial = otc_serial_dev_inst_new(device_path, serialcomm);
		...
		otc_info("PSLab device found: %s on port: %s", version, device_path);

		sdi = g_new0(struct otc_dev_inst, 1);
		...
		for (i = 0; i < NUM_ANALOG_CHANNELS; i++) {
			ch = otc_channel_new(sdi, analog_channels[i].index,OTC_CHANNEL_ANALOG,
					    TRUE, analog_channels[i].name);
			cp = g_new0(struct channel_priv, 1);
			cg = g_new0(struct otc_channel_group, 1);
...
static void select_range(const struct otc_channel_group *cg, uint8_t idx)
{
	uint16_t gain = GAIN_VALUES[idx];
	((struct channel_priv *)(((struct otc_channel *)(cg->channels->data))->priv))->gain = gain;
	otc_info("Set gain %d on channel %s with range %lu V",
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	if (!cg) {
		...
		case OTC_CONF_TRIGGER_SOURCE:
			...
			if (assign_channel(name, &devc->trigger_channel, sdi->channels) != OTC_OK)
				return OTC_ERR_ARG;
			break;
		case OTC_CONF_TRIGGER_LEVEL:
			devc->trigger_enabled = TRUE;
...
static int configure_channels(const struct otc_dev_inst *sdi)
{
	...
	for (l = sdi->channels; l; l = l->next) {
		...
		if (ch->enabled) {
			...
			otc_info("enabled channels: {} %s", ch->name);
		}
	}
	return OTC_OK;
}
...
{
	if (channels > 4) {
		otc_err("Number of channels to sample must be 1, 2, 3, or 4");
		return OTC_ERR_ARG;
	}

	if (samples > (MAX_SAMPLES/channels)) {
		otc_err("Invalid number of samples");
		return OTC_ERR_ARG;
	}

	if (samplerate > lookup_maximum_samplerate(channels, trigger)) {
		otc_err("Samplerate must be less than %lu",
...
static void configure_oscilloscope(const struct otc_dev_inst *sdi)
{
	GSList *l;
	struct dev_context *devc;
	struct otc_channel *ch;
...
OTC_REGISTER_DEV_DRIVER(pslab_driver_info);
