...
static const uint64_t samplerates[1] = { OTC_MHZ(100) };
...
static int dev_open(struct otc_dev_inst *sdi)
{
	...
	if ((ret = scanaplus_get_device_id(devc)) < 0) {
		otc_err("Failed to get ScanaPLUS device ID: %d.", ret);
		goto err_dev_open_close_ftdic;
	}
	otc_dbg("Received ScanaPLUS device ID successfully: %02x %02x %02x.",
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_SAMPLERATE:
		...
		*data = g_variant_new_uint64(OTC_MHZ(100));
		break;
	default:
		return OTC_ERR_NA;
	}
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	switch (key) {
	case OTC_CONF_SAMPLERATE:
		if (g_variant_get_uint64(data) != OTC_MHZ(100)) {
			otc_err("ScanaPLUS only supports samplerate = 100MHz.");
			return OTC_ERR_ARG;
		}
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	otc_session_source_add(sdi->session, -1, 0, 0, scanaplus_receive_data, (void *)sdi);
...
static struct otc_dev_driver ikalogic_scanaplus_driver_info = {
...
OTC_REGISTER_DEV_DRIVER(ikalogic_scanaplus_driver_info);
