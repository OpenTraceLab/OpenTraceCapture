...
static void send_samples(const struct otc_dev_inst *sdi, uint64_t samples_to_send)
{
	...
	otc_spew("Sending %" PRIu64 " samples.", samples_to_send);
...
OTC_PRIV int scanaplus_get_device_id(struct dev_context *devc)
{
	...
	if ((ret = ftdi_read_eeprom_location(devc->ftdic, 16, &val1)) < 0) {
		otc_err("Failed to read EEPROM index 16 (%d): %s.",
		       ret, ftdi_get_error_string(devc->ftdic));
		return OTC_ERR;
	}
	if ((ret = ftdi_read_eeprom_location(devc->ftdic, 17, &val2)) < 0) {
		otc_err("Failed to read EEPROM index 17 (%d): %s.",
		       ret, ftdi_get_error_string(devc->ftdic));
		return OTC_ERR;
	}
...
OTC_PRIV int scanaplus_receive_data(int fd, int revents, void *cb_data)
{
	...
	if (devc->compressed_bytes_ignored < COMPRESSED_BUF_SIZE) {
		...
		otc_spew("Ignoring first 64kB chunk of data.");
	...
	max = (OTC_MHZ(100) / 1000) * devc->limit_msec;
	...
	if (devc->limit_samples && (n >= devc->limit_samples)) {
		...
		otc_info("Requested time limit reached.");
		otc_dev_acquisition_stop(sdi);
		return TRUE;
	} else {
