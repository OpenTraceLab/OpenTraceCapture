...
static const uint32_t devopts[] = {
	...
	OTC_CONF_PHASE | OTC_CONF_GET | OTC_CONF_SET,
};
...
static const uint32_t devopts_cg[] = {
	...
	OTC_CONF_DUTY_CYCLE | OTC_CONF_GET | OTC_CONF_SET | OTC_CONF_LIST,
};
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	ser = otc_serial_dev_inst_new(conn, serialcomm);
	if (!ser)
		return devices;
	ret = serial_open(ser, SERIAL_RDWR);
	if (ret != OTC_OK) {
		otc_serial_dev_inst_free(ser);
		return devices;
	}
	...
	for (idx = 0; idx < MAX_GEN_CHANNELS; idx++) {
		...
		cg = otc_channel_group_new(sdi, cg_name, NULL);
		(void)cg;
		ch = otc_channel_new(sdi, ch_idx,
			OTC_CHANNEL_ANALOG, FALSE, cg_name);
...
static struct otc_dev_driver juntek_jds6600_driver_info = {
	.name = "juntek-jds6600",
...
OTC_REGISTER_DEV_DRIVER(juntek_jds6600_driver_info);
