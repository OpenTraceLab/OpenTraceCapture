...
static struct otc_dev_driver mooshimeter_dmm_driver_info;
...
static const uint32_t devopts[] = {
	...
	OTC_CONF_LIMIT_MSEC | OTC_CONF_SET | OTC_CONF_LIST,
	OTC_CONF_AVG_SAMPLES | OTC_CONF_GET | OTC_CONF_SET | OTC_CONF_LIST,
	OTC_CONF_SAMPLERATE | OTC_CONF_GET | OTC_CONF_SET | OTC_CONF_LIST,
	OTC_CONF_CHANNEL_CONFIG | OTC_CONF_SET,
};

static void init_dev(struct otc_dev_inst *sdi)
{
	...
	chan = otc_channel_new(sdi, 0, OTC_CHANNEL_ANALOG, TRUE, "CH1");
	devc->channel_meaning[0].mq = OTC_MQ_CURRENT;
	devc->channel_meaning[0].unit = OTC_UNIT_AMPERE;
	devc->channel_meaning[0].mqflags = OTC_MQFLAG_DC;
	devc->channel_meaning[0].channels = g_slist_prepend(NULL, chan);

	chan = otc_channel_new(sdi, 1, OTC_CHANNEL_ANALOG, TRUE, "CH2");
	devc->channel_meaning[1].mq = OTC_MQ_VOLTAGE;
	devc->channel_meaning[1].unit = OTC_UNIT_VOLT;
	devc->channel_meaning[1].mqflags = OTC_MQFLAG_DC;
	devc->channel_meaning[1].channels = g_slist_prepend(NULL, chan);

	chan = otc_channel_new(sdi, 2, OTC_CHANNEL_ANALOG, FALSE, "P");
	devc->channel_meaning[2].mq = OTC_MQ_POWER;
	devc->channel_meaning[2].unit = OTC_UNIT_WATT;
	devc->channel_meaning[2].mqflags = OTC_MQFLAG_RMS;
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	ret = otc_bt_config_addr_remote(desc, conn);
	if (ret < 0)
		goto err;
	 ...
	 */
	ret = otc_bt_config_notify(desc, 0x0015, 0x0012, 0x0016, 0x0001, 0);
	if (ret < 0)
		goto err;
...
static int set_channel1_mean(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
	devc->channel_meaning[0].mqflags &= ~OTC_MQFLAG_RMS;
	devc->channel_meaning[0].mqflags |= OTC_MQFLAG_DC;
...
static int set_channel1_rms(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
	devc->channel_meaning[0].mqflags &= ~OTC_MQFLAG_DC;
	devc->channel_meaning[0].mqflags |= OTC_MQFLAG_RMS;
...
static int set_channel1_buffer(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
	devc->channel_meaning[0].mqflags &= ~(OTC_MQFLAG_DC | OTC_MQFLAG_RMS);

	return mooshimeter_dmm_set_chooser(sdi, "CH1:ANALYSIS",
		"CH1:ANALYSIS:BUFFER");
}

static int set_channel2_mean(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
	devc->channel_meaning[1].mqflags &= ~OTC_MQFLAG_RMS;
	devc->channel_meaning[1].mqflags |= OTC_MQFLAG_DC;
...
static int set_channel2_rms(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
	devc->channel_meaning[1].mqflags &= ~OTC_MQFLAG_DC;
	devc->channel_meaning[1].mqflags |= OTC_MQFLAG_RMS;
...
static int set_channel2_buffer(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;

	devc->channel_meaning[1].mqflags &= ~(OTC_MQFLAG_DC | OTC_MQFLAG_RMS);

	return mooshimeter_dmm_set_chooser(sdi, "CH2:ANALYSIS",
		"CH2:ANALYSIS:BUFFER");
}

static void autorange_channel1_current(const struct otc_dev_inst *sdi,
	float value)
{
...
static int configure_channel1_current(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
	...
	if (range <= 0) {
		...
		devc->channel_meaning[0].mqflags |= OTC_MQFLAG_AUTORANGE;
	} else {
		devc->channel_autorange[0] = NULL;
		devc->channel_meaning[0].mqflags &= ~OTC_MQFLAG_AUTORANGE;
	}

	devc->channel_meaning[0].mqflags &= ~OTC_MQFLAG_DIODE;
	devc->channel_meaning[0].mq = OTC_MQ_CURRENT;
	devc->channel_meaning[0].unit = OTC_UNIT_AMPERE;
...
static void autorange_channel1_temperature(const struct otc_dev_inst *sdi,
	float value)
{
	mooshimeter_dmm_set_autorange(sdi, "CH1:RANGE_I",
...
static int configure_channel1_temperature(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
	...
	devc->channel_meaning[0].mqflags &= ~OTC_MQFLAG_DIODE;
	devc->channel_meaning[0].mq = OTC_MQ_TEMPERATURE;
	devc->channel_meaning[0].unit = OTC_UNIT_KELVIN;

	return OTC_OK;
}

static void autorange_channel1_auxv(const struct otc_dev_inst *sdi,
	float value)
{
	mooshimeter_dmm_set_autorange(sdi, "CH1:RANGE_I",
		"SHARED:AUX_V", value);
...
static int configure_channel1_auxv(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
	...
	devc->channel_meaning[0].mqflags &= ~OTC_MQFLAG_DIODE;
	devc->channel_meaning[0].mq = OTC_MQ_VOLTAGE;
	devc->channel_meaning[0].unit = OTC_UNIT_VOLT;

	return OTC_OK;
}

static void autorange_channel1_resistance(const struct otc_dev_inst *sdi,
	float value)
{
	mooshimeter_dmm_set_autorange(sdi, "CH1:RANGE_I",
...
static int configure_channel1_resistance(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
	...
	devc->channel_meaning[0].mqflags &= ~OTC_MQFLAG_DIODE;
	devc->channel_meaning[0].mq = OTC_MQ_RESISTANCE;
	devc->channel_meaning[0].unit = OTC_UNIT_OHM;

	return OTC_OK;
}

static void autorange_channel1_diode(const struct otc_dev_inst *sdi,
	float value)
{
	mooshimeter_dmm_set_autorange(sdi, "CH1:RANGE_I",
		"SHARED:DIODE", value);
...
static int configure_channel1_diode(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
...
static void autorange_channel2_voltage(const struct otc_dev_inst *sdi,
	float value)
{
...
static int configure_channel2_voltage(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
	...
	if (range <= 0) {
		...
		devc->channel_meaning[1].mqflags |= OTC_MQFLAG_AUTORANGE;
	} else {
		devc->channel_autorange[1] = NULL;
		devc->channel_meaning[1].mqflags &= ~OTC_MQFLAG_AUTORANGE;
	}

	devc->channel_meaning[1].mqflags &= ~OTC_MQFLAG_DIODE;
	devc->channel_meaning[1].mq = OTC_MQ_VOLTAGE;
	devc->channel_meaning[1].unit = OTC_UNIT_VOLT;
...
static void autorange_channel2_temperature(const struct otc_dev_inst *sdi,
	float value)
{
	mooshimeter_dmm_set_autorange(sdi, "CH2:RANGE_I",
...
static int configure_channel2_temperature(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
	...
	devc->channel_meaning[1].mqflags &= ~OTC_MQFLAG_DIODE;
	devc->channel_meaning[1].mq = OTC_MQ_TEMPERATURE;
	devc->channel_meaning[1].unit = OTC_UNIT_CELSIUS;

	return OTC_OK;
}

static void autorange_channel2_auxv(const struct otc_dev_inst *sdi,
	float value)
{
	mooshimeter_dmm_set_autorange(sdi, "CH2:RANGE_I",
		"SHARED:AUX_V", value);
...
static int configure_channel2_auxv(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
...
static void autorange_channel2_resistance(const struct otc_dev_inst *sdi,
	float value)
{
	mooshimeter_dmm_set_autorange(sdi, "CH2:RANGE_I",
		"SHARED:RESISTANCE", value);
...
static int configure_channel2_resistance(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
	...
	devc->channel_meaning[1].mqflags &= ~OTC_MQFLAG_DIODE;
	devc->channel_meaning[1].mq = OTC_MQ_RESISTANCE;
	devc->channel_meaning[1].unit = OTC_UNIT_OHM;

	return OTC_OK;
}

static void autorange_channel2_diode(const struct otc_dev_inst *sdi,
	float value)
{
	mooshimeter_dmm_set_autorange(sdi, "CH2:RANGE_I",
		"SHARED:DIODE", value);
...
static int configure_channel2_diode(const struct otc_dev_inst *sdi,
	float range)
{
	struct dev_context *devc = sdi->priv;
	int ret;
 ...
 * 	Buffer, Samples
 */
static int apply_channel_config(const struct otc_dev_inst *sdi,
	const char *config)
{
	...
	if (parameters[0] && parameters[0][0]) {
		...
		if (!g_ascii_strncasecmp(param, "Resistance", 10) ||
			...
			*parameters[0]) {
			...
			otc_info("Unrecognized mode for CH1: %s.", param);
			ret = configure_channel1_current(sdi, range);
			if (ret != OTC_OK)
				goto err_free_parameters;
		}
	...
	if (parameters[0] && parameters[0][0]) {
		...
		if (!g_ascii_strncasecmp(param, "Resistance", 10) ||
			...
			otc_info("Unrecognized mode for CH2: %s.", param);
			ret = configure_channel2_voltage(sdi, range);
			if (ret != OTC_OK)
				goto err_free_parameters;
		}
...
static int dev_acquisition_stop(struct otc_dev_inst *sdi)
{
	...
	otc_session_source_remove(sdi->session, -2);
...
OTC_REGISTER_DEV_DRIVER(mooshimeter_dmm_driver_info);
