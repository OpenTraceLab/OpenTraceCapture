...
static struct otc_dev_inst *dev_inst_new(const struct model_info *model)
{
 ...
 */
static int drain_usb(struct otc_usb_dev_inst *usb, unsigned int endpoint)
{
	...
	if (ret != LIBUSB_SUCCESS && ret != LIBUSB_ERROR_TIMEOUT) {
		otc_err("Failed to drain USB endpoint %u: %s.",
		       endpoint & (LIBUSB_ENDPOINT_IN - 1),
		       libusb_error_name(ret));
		return OTC_ERR;
	}
	if (drained > 0) {
		otc_warn("Drained %d bytes from USB endpoint %u.",
...
static int dev_open(struct otc_dev_inst *sdi)
{
	...
	if (ret == OTC_OK && devc->short_transfer_quirk)
		otc_warn("Short transfer quirk detected! "
...
static int dev_close(struct otc_dev_inst *sdi)
{
	...
	if (devc->acquisition) {
		otc_err("Cannot close device during acquisition!");
	...
	ret = (*devc->model->apply_fpga_config)(sdi);
	if (ret != OTC_OK)
		otc_warn("Unable to shut down device.");
...
static int has_devopt(const struct model_info *model, uint32_t key)
{
	...
	for (i = 0; i < model->num_devopts; i++) {
		if ((model->devopts[i] & (OTC_CONF_MASK | key)) == key)
			return TRUE;
	}
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	if (!has_devopt(devc->model, key | OTC_CONF_GET))
		return OTC_ERR_NA;
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	if (!has_devopt(devc->model, key | OTC_CONF_SET))
		return OTC_ERR_NA;
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	if (!devc)
		return OTC_ERR_ARG;
	if (!has_devopt(devc->model, key | OTC_CONF_LIST))
		return OTC_ERR_NA;
...
static int dev_acquisition_stop(struct otc_dev_inst *sdi)
{
	...
	if (devc->state != STATE_IDLE && !devc->cancel_requested) {
		devc->cancel_requested = TRUE;
		otc_dbg("Requesting cancel.");
	}

	return OTC_OK;
}

static struct otc_dev_driver sysclk_lwla_driver_info = {
	.name = "sysclk-lwla",
...
OTC_REGISTER_DEV_DRIVER(sysclk_lwla_driver_info);
