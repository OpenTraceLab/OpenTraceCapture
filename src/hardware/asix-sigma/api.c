...
static const uint32_t devopts[] = {
	...
	/* Consider OTC_CONF_TRIGGER_PATTERN (OTC_T_STRING, GET/SET) support. */
};
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	drvc = di->context;
	usbctx = drvc->otc_ctx->libusb_ctx;
	...
	conn_devices = NULL;
	if (conn)
		conn_devices = otc_usb_find(usbctx, conn);
	...
	for (devidx = 0; devlist[devidx]; devidx++) {
		...
		if (!des.iSerialNumber) {
			otc_warn("Cannot get serial number (index 0).");
			continue;
		}
		ret = libusb_open(devitem, &hdl);
		if (ret < 0) {
			otc_warn("Cannot open USB device %04x.%04x: %s.",
		...
		if (ret < 0) {
			otc_warn("Cannot get serial number (%s).",
				libusb_error_name(ret));
			libusb_close(hdl);
			continue;
		}
		 ...
		 */
		ret = otc_atoul_base(serno_txt, &serno_num, &end, 16);
		if (ret != OTC_OK || !end || *end) {
			otc_warn("Cannot interpret serial number %s.", serno_txt);
			continue;
		}
		...
		case 0xa601:
			...
			dev_text = "SIGMA";
			otc_info("Found SIGMA, serno %s.", serno_txt);
			break;
		case 0xa602:
			dev_type = ASIX_TYPE_SIGMA;
			dev_text = "SIGMA2";
			otc_info("Found SIGMA2, serno %s.", serno_txt);
			break;
		case 0xa603:
			dev_type = ASIX_TYPE_OMEGA;
			dev_text = "OMEGA";
			otc_info("Found OMEGA, serno %s.", serno_txt);
			if (!ASIX_WITH_OMEGA) {
				otc_warn("OMEGA support is not implemented yet.");
				continue;
			}
			break;
		default:
			otc_warn("Unknown serno %s, skipping.", serno_txt);
			continue;
		}
		...
		otc_sw_limits_init(&devc->limit.config);
...
static int dev_open(struct otc_dev_inst *sdi)
{
	...
	if (devc->id.type == ASIX_TYPE_OMEGA && !ASIX_WITH_OMEGA) {
		otc_err("OMEGA support is not implemented yet.");
		return OTC_ERR_NA;
	}
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_LIMIT_SAMPLES:
		return otc_sw_limits_config_get(&devc->limit.config, key, data);
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_SAMPLERATE:
		...
		if (have_rate != want_rate) {
			...
			text_want = otc_samplerate_string(want_rate);
			text_have = otc_samplerate_string(have_rate);
			otc_info("Adjusted samplerate %s to %s.",
				text_want, text_have);
			g_free(text_want);
			g_free(text_have);
		}
	...
	case OTC_CONF_LIMIT_MSEC:
	case OTC_CONF_LIMIT_SAMPLES:
		return otc_sw_limits_config_set(&devc->limit.config, key, data);
	case OTC_CONF_CAPTURE_RATIO:
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	if (ret != OTC_OK) {
		otc_err("Could not configure triggers.");
		return ret;
	}
	...
	if (devc->clock.use_ext_clock) {
		if (devc->clock.samplerate != OTC_MHZ(50))
			otc_info("External clock, forcing 50MHz samplerate.");
		devc->clock.samplerate = OTC_MHZ(50);
	}
	...
	} else if (devc->clock.samplerate <= OTC_MHZ(50)) {
	...
	if (devc->clock.samplerate > OTC_MHZ(50)) {
		...
		async = 0;
		div = OTC_MHZ(50) / devc->clock.samplerate - 1;
		if (devc->clock.use_ext_clock) {
	...
	/* Add capture source. */
	ret = otc_session_source_add(sdi->session, -1, 0, 10,
...
static int dev_acquisition_stop(struct otc_dev_inst *sdi)
{
	...
	if (devc->state == SIGMA_CAPTURE) {
		...
		(void)otc_session_source_remove(sdi->session, -1);
	}

	return OTC_OK;
}

static struct otc_dev_driver asix_sigma_driver_info = {
	.name = "asix-sigma",
...
OTC_REGISTER_DEV_DRIVER(asix_sigma_driver_info);
