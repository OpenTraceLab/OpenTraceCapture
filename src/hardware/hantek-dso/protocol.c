...
static int send_begin(const struct otc_dev_inst *sdi)
{
	...
	otc_dbg("Sending CTRL_BEGINCOMMAND.");
	...
	if ((ret = libusb_control_transfer(usb->devhdl,
		...
		otc_err("Failed to send begincommand: %s.",
...
static int send_bulkcmd(const struct otc_dev_inst *sdi, uint8_t *cmdstring, int cmdlen)
{
	struct otc_usb_dev_inst *usb;
	int ret, tmp;
	...
	for (i = 0; devlist[i]; i++) {
		...
		if (!(err = libusb_open(devlist[i], &usb->devhdl))) {
			...
			if (!(devc->epin_maxpacketsize = dso_getmps(devlist[i])))
				otc_err("Wrong endpoint profile.");
			else {
...
static int get_channel_offsets(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	struct otc_usb_dev_inst *usb;
	GString *gs;
	...
	otc_dbg("Getting channel offsets.");
	...
	if (ret != sizeof(devc->channel_levels)) {
		otc_err("Failed to get channel offsets: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
...
static void dso2250_set_triggerpos(int value, int long_buffer, uint8_t dest[], int offset)
{
	...
	otc_dbg("2250 trigger pos: %3d%% * [0x%x,0x%x] == 0x%x", value, min, max, tmp);
...
static int dso2250_set_trigger_samplerate(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	struct otc_usb_dev_inst *usb;
	int ret, tmp;
	...
	otc_dbg("Trigger source %s.", devc->triggersource ? : "<none>");
	if (!devc->triggersource)
		...
		otc_err("Invalid trigger source: '%s'.", devc->triggersource);
		return OTC_ERR_ARG;
	}
	cmdstring[2] = tmp;

	otc_dbg("Trigger slope: %d.", devc->triggerslope);
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
		...
		otc_err("Failed to set trigger/samplerate: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}

	/* Frame size */
	otc_dbg("Frame size: %d.", devc->framesize);
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
		...
		otc_err("Failed to set record length: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	if (devc->samplerate > base) {
		/* Timebase fast */
		otc_err("Sample rate > 100MHz not yet supported.");
		return OTC_ERR_ARG;
	}
	...
	if (tmp > 1) {
		...
		tmp -= 2;
		tmp = ~tmp;
		otc_dbg("Down sampler value: 0x%x.", tmp & 0xffff);
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
			cmdstring, 8, &tmp, 100)) != 0) {
		otc_err("Failed to set sample rate: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
	otc_dbg("Sent CMD_2250_SET_SAMPLERATE.");

	/* Enabled channels: 00=CH1, 01=CH2, 10=both. */
	memset(cmdstring, 0, sizeof(cmdstring));
	cmdstring[0] = CMD_2250_SET_CHANNELS;
	otc_dbg("Channels: CH1=%d, CH2=%d.", devc->ch_enabled[0], devc->ch_enabled[1]);
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
			cmdstring, 4, &tmp, 100)) != 0) {
		otc_err("Failed to set channels: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
	otc_dbg("Sent CMD_2250_SET_CHANNELS.");

	/* Trigger slope: 0=positive, 1=negative. */
	memset(cmdstring, 0, sizeof(cmdstring));
	cmdstring[0] = CMD_2250_SET_TRIGGERPOS_AND_BUFFER;

	otc_dbg("Capture ratio: %" PRIu64 ".", devc->capture_ratio);
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
		...
		otc_err("Failed to set trigger position: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
...
OTC_PRIV int dso_set_trigger_samplerate(const struct otc_dev_inst *sdi)
{
	...
	otc_dbg("Preparing CMD_SET_TRIGGER_SAMPLERATE.");
	...
	/* Timebase fast */
	otc_dbg("Time base index: %d.", devc->timebase);
	if (devc->framesize == FRAMESIZE_SMALL) {
		...
		if (devc->timebase < TIME_40us) {
			otc_err("Timebase < 40us only supported with 10K buffer.");
			return OTC_ERR_ARG;
		}
		else if (devc->timebase == TIME_40us)
			tmp = 0;
	...
	otc_dbg("Sent CMD_SET_TRIGGER_SAMPLERATE.");

	return OTC_OK;
}

static int dso_set_filters(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	struct otc_usb_dev_inst *usb;
	int ret, tmp;
	...
	otc_dbg("Preparing CMD_SET_FILTERS.");
	...
	cmdstring[1] = 0x0f;
	if (devc->filter[0]) {
		otc_dbg("Turning on CH1 filter.");
		cmdstring[2] |= 0x80;
	}
	if (devc->filter[1]) {
		otc_dbg("Turning on CH2 filter.");
		cmdstring[2] |= 0x40;
	}
	/*
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
			cmdstring, sizeof(cmdstring), &tmp, 100)) != 0) {
		otc_err("Failed to set filters: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
	otc_dbg("Sent CMD_SET_FILTERS.");

	return OTC_OK;
}

static int dso_set_voltage(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	struct otc_usb_dev_inst *usb;
	int ret, tmp;
	...
	otc_dbg("Preparing CMD_SET_VOLTAGE.");
	...
	otc_dbg("CH1 vdiv index: %d.", devc->voltage[0]);
	...
	otc_dbg("CH2 vdiv index: %d.", devc->voltage[1]);
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
			cmdstring, sizeof(cmdstring), &tmp, 100)) != 0) {
		otc_err("Failed to set voltage: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
	otc_dbg("Sent CMD_SET_VOLTAGE.");

	return OTC_OK;
}

static int dso_set_relays(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
	struct otc_usb_dev_inst *usb;
	GString *gs;
	...
	otc_dbg("Preparing CTRL_SETRELAYS.");
	...
	otc_dbg("CH1 coupling: %d.", devc->coupling[0]);
	...
	otc_dbg("CH2 coupling: %d.", devc->coupling[1]);
	...
	if ((ret = libusb_control_transfer(usb->devhdl,
		...
		otc_err("Failed to set relays: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
	otc_dbg("Sent CTRL_SETRELAYS.");
...
OTC_PRIV int dso_set_voffsets(const struct otc_dev_inst *sdi)
{
	...
	otc_dbg("Preparing CTRL_SETOFFSET.");
	...
	otc_dbg("CH1 offset: %3.2f (%.2x%.2x).", devc->voffset_ch1,
	       offsets[0], offsets[1]);
	...
	otc_dbg("CH2 offset: %3.2f (%.2x%.2x).", devc->voffset_ch2,
	       offsets[2], offsets[3]);
	...
	otc_dbg("Trigger offset: %3.2f (%.2x%.2x).", devc->voffset_trigger,
			offsets[4], offsets[5]);

	if ((ret = libusb_control_transfer(usb->devhdl,
			LIBUSB_REQUEST_TYPE_VENDOR, CTRL_SETOFFSET,
			0, 0, offsets, sizeof(offsets), 100)) != sizeof(offsets)) {
		otc_err("Failed to set offsets: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
	otc_dbg("Sent CTRL_SETOFFSET.");
...
OTC_PRIV int dso_enable_trigger(const struct otc_dev_inst *sdi)
{
	...
	otc_dbg("Sending CMD_ENABLE_TRIGGER.");
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
			cmdstring, sizeof(cmdstring), &tmp, 100)) != 0) {
		otc_err("Failed to enable trigger: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
...
OTC_PRIV int dso_force_trigger(const struct otc_dev_inst *sdi)
{
	...
	otc_dbg("Sending CMD_FORCE_TRIGGER.");
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_OUT,
			cmdstring, sizeof(cmdstring), &tmp, 100)) != 0) {
		otc_err("Failed to force trigger: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
...
{
	otc_dbg("Initializing DSO.");

	if (get_channel_offsets(sdi) != OTC_OK)
		return OTC_ERR;

	if (dso_set_trigger_samplerate(sdi) != OTC_OK)
		return OTC_ERR;

	if (dso_set_filters(sdi) != OTC_OK)
		return OTC_ERR;

	if (dso_set_voltage(sdi) != OTC_OK)
		return OTC_ERR;

	if (dso_set_relays(sdi) != OTC_OK)
		return OTC_ERR;
		...
		uint8_t *capturestate, uint32_t *trigger_offset)
{
	...
	otc_dbg("Sending CMD_GET_CAPTURESTATE.");
	...
	if ((ret = send_bulkcmd(sdi, cmdstring, sizeof(cmdstring))) != OTC_OK) {
		otc_dbg("Failed to send get_capturestate command: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	if ((ret = libusb_bulk_transfer(usb->devhdl, DSO_EP_IN,
			inbuf, 512, &tmp, 100)) != 0) {
		otc_dbg("Failed to get capturestate: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
...
OTC_PRIV int dso_capture_start(const struct otc_dev_inst *sdi)
{
	...
	otc_dbg("Sending CMD_CAPTURE_START.");

	cmdstring[0] = CMD_CAPTURE_START;
	cmdstring[1] = 0;

	if ((ret = send_bulkcmd(sdi, cmdstring, sizeof(cmdstring))) != OTC_OK) {
		otc_err("Failed to send capture_start command: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
		...
		libusb_transfer_cb_fn cb)
{
	...
	otc_dbg("Sending CMD_GET_CHANNELDATA.");
	...
	if ((ret = send_bulkcmd(sdi, cmdstring, sizeof(cmdstring))) != OTC_OK) {
		otc_err("Failed to get channel data: %s.",
		       libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	otc_dbg("Queueing up %d transfers.", num_transfers);
