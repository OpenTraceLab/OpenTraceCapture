...
#define DS_MAX_LOGIC_DEPTH		OTC_MHZ(16)
#define DS_MAX_LOGIC_SAMPLERATE		OTC_MHZ(100)
				  ...
				  struct version_info *vi)
{
	...
	if (ret < 0) {
		otc_err("Unable to get version info: %s.",
...
static int command_get_revid_version(struct otc_dev_inst *sdi, uint8_t *revid)
{
	...
	if (ret < 0) {
		otc_err("Unable to get REVID: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
...
static int command_start_acquisition(const struct otc_dev_inst *sdi)
{
	...
	if (ret < 0) {
		otc_err("Failed to send start command: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
...
static int command_stop_acquisition(const struct otc_dev_inst *sdi)
{
	struct otc_usb_dev_inst *usb;
	struct dslogic_mode mode;
	int ret;
	...
	if (ret < 0) {
		otc_err("Failed to send stop command: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
...
OTC_PRIV int dslogic_fpga_firmware_upload(const struct otc_dev_inst *sdi)
{
	...
	if (!strcmp(devc->profile->model, "DSLogic")) {
		...
		otc_err("Failed to select FPGA firmware.");
		return OTC_ERR;
	}

	otc_dbg("Uploading FPGA firmware '%s'.", name);

	result = otc_resource_open(drvc->otc_ctx, &bitstream,
	...
	if ((ret = libusb_control_transfer(usb->devhdl, LIBUSB_REQUEST_TYPE_VENDOR |
		...
		otc_err("Failed to upload FPGA firmware: %s.", libusb_error_name(ret));
	...
	while (1) {
		...
		if ((ret = libusb_bulk_transfer(usb->devhdl, 2 | LIBUSB_ENDPOINT_OUT,
			...
			otc_err("Unable to configure FPGA firmware: %s.",
					libusb_error_name(ret));
			result = OTC_ERR;
			break;
		}
		sum += transferred;
		otc_spew("Uploaded %" PRIu64 "/%" PRIu64 " bytes.",
			sum, bitstream.size);

		if (transferred != chunksize) {
			otc_err("Short transfer while uploading FPGA firmware.");
			result = OTC_ERR;
			break;
		}
	}
	...
	if (result == OTC_OK)
		otc_dbg("FPGA firmware upload done.");

	return result;
}

static unsigned int enabled_channel_count(const struct otc_dev_inst *sdi)
{
	unsigned int count = 0;
	for (const GSList *l = sdi->channels; l; l = l->next) {
		const struct otc_channel *const probe = (struct otc_channel *)l->data;
		if (probe->enabled)
			count++;
	}
	return count;
...
static uint16_t enabled_channel_mask(const struct otc_dev_inst *sdi)
{
	unsigned int mask = 0;
	for (const GSList *l = sdi->channels; l; l = l->next) {
		const struct otc_channel *const probe = (struct otc_channel *)l->data;
 ...
 */
static bool set_trigger(const struct otc_dev_inst *sdi, struct fpga_config *cfg)
{
	...
	if (!(trigger = otc_session_trigger_get(sdi->session))) {
		otc_dbg("No session trigger found");
		return false;
	}
...
static int fpga_configure(const struct otc_dev_inst *sdi)
{
	const struct dev_context *const devc = sdi->priv;
	const struct otc_usb_dev_inst *const usb = sdi->conn;
	uint8_t c[3];
	...
	if (ret < 0) {
		otc_err("Failed to send FPGA configure command: %s.",
			libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	if (ret < 0 || transferred != len) {
		otc_err("Failed to send FPGA configuration: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
...
OTC_PRIV int dslogic_set_voltage_threshold(const struct otc_dev_inst *sdi, double threshold)
{
	...
	if (ret < 0) {
		otc_err("Unable to set voltage-threshold register: %s.",
		libusb_error_name(ret));
		return OTC_ERR;
	}
...
OTC_PRIV int dslogic_dev_open(struct otc_dev_inst *sdi, struct otc_dev_driver *di)
{
	...
	for (i = 0; i < device_count; i++) {
		...
		if (ret != OTC_OK) {
			otc_err("Failed to get firmware version.");
			break;
		}

		ret = command_get_revid_version(sdi, &revid);
		if (ret != OTC_OK) {
			otc_err("Failed to get REVID.");
			break;
		}
		...
		if (vi.major != DSLOGIC_REQUIRED_VERSION_MAJOR) {
			otc_err("Expected firmware version %d.x, "
		...
		otc_info("Opened device on %d.%d (logical) / %s (physical), "
			"interface %d, firmware %d.%d.",
			usb->bus, usb->address, connection_id,
			USB_INTERFACE, vi.major, vi.minor);

		otc_info("Detected REVID=%d, it's a Cypress CY7C68013%s.",
...
static void LIBUSB_CALL receive_transfer(struct libusb_transfer *transfer)
{
	...
	if (!devc->limit_samples || devc->sent_samples < devc->limit_samples) {
		 ...
		 * channel.
		 *
		 * Because opentracelab's internal representation is bit-interleaved channels
		...
		if (transfer->actual_length % (DSLOGIC_ATOMIC_BYTES * channel_count) != 0)
			otc_err("Invalid transfer length!");
...
static size_t to_bytes_per_ms(const struct otc_dev_inst *sdi)
{
	const struct dev_context *const devc = sdi->priv;
...
static size_t get_buffer_size(const struct otc_dev_inst *sdi)
{
	/*
...
static unsigned int get_number_of_transfers(const struct otc_dev_inst *sdi)
{
...
static unsigned int get_timeout(const struct otc_dev_inst *sdi)
{
...
static int start_transfers(const struct otc_dev_inst *sdi)
{
	const size_t channel_count = enabled_channel_count(sdi);
	...
	if (!devc->transfers) {
		otc_err("USB transfers malloc failed.");
	...
	if (!devc->deinterleave_buffer) {
		otc_err("Deinterleave buffer malloc failed.");
		g_free(devc->deinterleave_buffer);
		return OTC_ERR_MALLOC;
	}
	...
	for (i = 0; i < num_transfers; i++) {
		if (!(buf = g_try_malloc(size))) {
			otc_err("USB transfer buffer malloc failed.");
			return OTC_ERR_MALLOC;
		}
		...
		otc_info("submitting transfer: %d", i);
...
static void LIBUSB_CALL trigger_receive(struct libusb_transfer *transfer)
{
	...
	if (transfer->status == LIBUSB_TRANSFER_CANCELLED) {
		otc_dbg("Trigger transfer canceled.");
		/* Terminate session. */
		std_session_send_df_end(sdi);
		usb_source_remove(sdi->session, devc->ctx);
		devc->num_transfers = 0;
		...
		otc_info("tpos real_pos %d ram_saddr %d cnt_h %d cnt_l %d", tpos->real_pos,
...
OTC_PRIV int dslogic_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	if ((ret = command_stop_acquisition(sdi)) != OTC_OK)
		return ret;

	if ((ret = fpga_configure(sdi)) != OTC_OK)
		return ret;

	if ((ret = command_start_acquisition(sdi)) != OTC_OK)
		return ret;

	otc_dbg("Getting trigger.");
	...
	if ((ret = libusb_submit_transfer(transfer)) < 0) {
		otc_err("Failed to request trigger: %s.", libusb_error_name(ret));
		libusb_free_transfer(transfer);
		g_free(tpos);
		return OTC_ERR;
	}
	...
	if (!devc->transfers) {
		otc_err("USB trigger_pos transfer malloc failed.");
		return OTC_ERR_MALLOC;
	}
