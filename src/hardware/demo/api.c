...
/* Note: No spaces allowed because of opentracelab-cli. */
...
static const uint32_t devopts[] = {
	...
	OTC_CONF_AVG_SAMPLES | OTC_CONF_GET | OTC_CONF_SET,
...
static const uint32_t devopts_cg_analog_channel[] = {
	OTC_CONF_MEASURED_QUANTITY | OTC_CONF_GET | OTC_CONF_SET,
	OTC_CONF_PATTERN_MODE | OTC_CONF_GET | OTC_CONF_SET | OTC_CONF_LIST,
	OTC_CONF_AMPLITUDE | OTC_CONF_GET | OTC_CONF_SET,
	OTC_CONF_OFFSET | OTC_CONF_GET | OTC_CONF_SET,
};
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	struct otc_channel_group *cg, *acg;
	...
	devc->cur_samplerate = OTC_KHZ(200);
	...
	if (num_analog_channels > 0) {
		...
		acg = otc_channel_group_new(sdi, "Analog", NULL);

		for (i = 0; i < num_analog_channels; i++) {
			snprintf(channel_name, 16, "A%d", i);
			ch = otc_channel_new(sdi, i + num_logic_channels, OTC_CHANNEL_ANALOG,
					TRUE, channel_name);
			...
			ag->ch = ch;
			ag->mq = OTC_MQ_VOLTAGE;
			ag->mq_flags = OTC_MQFLAG_DC;
			ag->unit = OTC_UNIT_VOLT;
			ag->amplitude = DEFAULT_ANALOG_AMPLITUDE;
			ag->offset = DEFAULT_ANALOG_OFFSET;
			otc_analog_init(&ag->packet, &ag->encoding, &ag->meaning, &ag->spec, 2);
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_AVERAGING:
		...
		otc_dbg("%s averaging", devc->avg ? "Enabling" : "Disabling");
		break;
	case OTC_CONF_AVG_SAMPLES:
		devc->avg_samples = g_variant_get_uint64(data);
		otc_dbg("Setting averaging rate to %" PRIu64, devc->avg_samples);
		break;
	case OTC_CONF_MEASURED_QUANTITY:
		if (!cg)
			return OTC_ERR_CHANNEL_GROUP;
	...
	case OTC_CONF_PATTERN_MODE:
		...
		for (l = cg->channels; l; l = l->next) {
			...
			if (ch->type == OTC_CHANNEL_LOGIC) {
				if (logic_pattern == -1)
					return OTC_ERR_ARG;
				otc_dbg("Setting logic pattern to %s",
				...
				if (analog_pattern == -1)
					return OTC_ERR_ARG;
				otc_dbg("Setting analog pattern for channel %s to %s",
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	otc_dbg("num logic %zu, partial off %zu, mask 0x%02x.",
		devc->enabled_logic_channels,
		devc->first_partial_logic_index,
		devc->first_partial_logic_mask);

	otc_session_source_add(sdi->session, -1, 0, 100,
			demo_prepare_data, (struct otc_dev_inst *)sdi);
...
static struct otc_dev_driver demo_driver_info = {
	.name = "demo",
...
OTC_REGISTER_DEV_DRIVER(demo_driver_info);
