...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	for (i = 0; devlist[i]; i++) {
		...
		if (check_conf_profile(devlist[i])) {
			...
			otc_dbg("Found a Logic16 device.");
			...
			if (ezusb_upload_firmware(drvc->otc_ctx, devlist[i],
					USB_CONFIGURATION, FX2_FIRMWARE) == OTC_OK) {
				/* Store when this device's FW was updated. */
				devc->fw_updated = g_get_monotonic_time();
			} else {
				otc_err("Firmware upload failed, name %s.", FX2_FIRMWARE);
			}
			sdi->inst_type = OTC_INST_USB;
			sdi->conn = otc_usb_dev_inst_new(
...
static int logic16_dev_open(struct otc_dev_inst *sdi)
{
	...
	for (i = 0; i < device_count; i++) {
		...
		if ((ret = logic16_init_device(sdi)) != OTC_OK) {
			otc_err("Failed to init device.");
			break;
		}

		otc_info("Opened device on %d.%d (logical) / %s (physical), interface %d.",
...
static int dev_open(struct otc_dev_inst *sdi)
{
	...
	if (devc->fw_updated > 0) {
		...
		while (timediff_ms < MAX_RENUM_DELAY_MS) {
			if ((ret = logic16_dev_open(sdi)) == OTC_OK)
				break;
			g_usleep(100 * 1000);
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	if (!(devc->convbuffer = g_try_malloc(convsize))) {
		otc_err("Conversion buffer malloc failed.");
		return OTC_ERR_MALLOC;
	}
	...
	if ((ret = logic16_start_acquisition(sdi)) != OTC_OK) {
		abort_acquisition(devc);
		return ret;
	}
...
static struct otc_dev_driver saleae_logic16_driver_info = {
	.name = "saleae-logic16",
...
OTC_REGISTER_DEV_DRIVER(saleae_logic16_driver_info);
