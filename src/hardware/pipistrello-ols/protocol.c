...
OTC_PRIV int write_longcommand(struct dev_context *devc, uint8_t command, uint8_t *data)
{
	...
	otc_dbg("Sending cmd 0x%.2x data 0x%.2x%.2x%.2x%.2x.", command,
...
OTC_PRIV int p_ols_open(struct dev_context *devc)
{
	...
	if (ret < 0) {
		otc_err("Failed to set FTDI interface B (%d): %s", ret,
...
OTC_PRIV int pols_convert_trigger(const struct otc_dev_inst *sdi)
{
	...
	for (l = trigger->stages; l; l = l->next) {
		...
		for (m = stage->matches; m; m = m->next) {
			...
			if (match->match == OTC_TRIGGER_RISING || match->match == OTC_TRIGGER_FALLING)
...
OTC_PRIV struct otc_dev_inst *p_ols_get_metadata(uint8_t *buf, int bytes_read, struct dev_context *devc)
{
	...
	while (index < bytes_read) {
		...
		case 0:
			...
			otc_dbg("Got metadata key 0x%.2x value '%s'.",
			       key, tmp_str->str);
			switch (token) {
			case 0x01:
				/* Device name */
			...
			case 0x03:
				...
				break;
			default:
				otc_info("Unknown token 0x%.2x: '%s'",
					token, tmp_str->str);
				break;
			}
		...
		case 1:
			...
			otc_dbg("Got metadata key 0x%.2x value 0x%.8x.",
			       key, tmp_int);
			switch (token) {
			case 0x00:
				/* Number of usable channels */
				for (ui = 0; ui < tmp_int; ui++)
					otc_channel_new(sdi, ui, OTC_CHANNEL_LOGIC, TRUE,
			...
			case 0x04:
				...
				break;
			default:
				otc_info("Unknown token 0x%.2x: 0x%.8x.",
					token, tmp_int);
				break;
			}
			break;
		case 2:
			...
			tmp_c = buf[index++];
			otc_dbg("Got metadata key 0x%.2x value 0x%.2x.",
			       key, tmp_c);
			switch (token) {
			case 0x00:
				/* Number of usable channels */
			...
			case 0x01:
				...
				break;
			default:
				otc_info("Unknown token 0x%.2x: 0x%.2x.",
					token, tmp_c);
				break;
			}
			break;
...
OTC_PRIV int p_ols_set_samplerate(const struct otc_dev_inst *sdi,
		const uint64_t samplerate)
{
	...
	if (devc->cur_samplerate != samplerate)
		otc_info("Can't match samplerate %" PRIu64 ", using %"
...
OTC_PRIV int p_ols_receive_data(int fd, int revents, void *cb_data)
{
	...
	if ((devc->num_samples < devc->limit_samples) && (devc->cnt_samples < devc->max_samples)) {
		...
		if (bytes_read == 0) {
			otc_spew("Received 0 bytes, nothing to do.");
			return TRUE;
		}

		otc_dbg("Received %d bytes", bytes_read);
			...
			if ((devc->flag_reg & FLAG_DEMUX) && (devc->flag_reg & FLAG_RLE)) {
				...
				*/
				if (devc->num_bytes == num_channels * 2) {
					...
					otc_spew("Received sample pair 0x%.*x.", devc->num_bytes * 2, sample);
					...
					if (devc->sample[devc->num_bytes - 1] & 0x80) {
						...
						otc_dbg("RLE count: %u.", devc->rle_count * 2);
						devc->num_bytes = 0;
						continue;
					}
					...
					otc_spew("Expanded sample 1: 0x%.2x%.2x%.2x%.2x.",
					...
					otc_spew("Expanded sample 2: 0x%.2x%.2x%.2x%.2x.",
				...
				if (devc->num_bytes == num_channels) {
					...
					sample = devc->sample[0] | (devc->sample[1] << 8) \
							| (devc->sample[2] << 16) | (devc->sample[3] << 24);
					otc_spew("Received sample 0x%.*x.", devc->num_bytes * 2, sample);
					if (devc->flag_reg & FLAG_RLE) {
						...
						if (devc->sample[devc->num_bytes - 1] & 0x80) {
							...
							otc_dbg("RLE count: %u.", devc->rle_count);
							devc->num_bytes = 0;
							continue;
						}
					}
					...
					if (num_channels < 4) {
						...
						otc_spew("Expanded sample: 0x%.8x.", sample);
					}
