...
{
	const struct otc_dev_inst *sdi = data;
	...
	otc_info("Unified polling thread started");

	/* Build channel lists */
	for (GSList *l = sdi->channels; l; l = l->next) {
		struct otc_channel *ch = l->data;
		if (!ch->enabled)
			continue;
	...
	while (TRUE) {
		...
		if (devc->limit_samples > 0 && devc->num_samples >= devc->limit_samples) {
			g_mutex_unlock(&devc->polling_mutex);
			otc_info("Sample limit reached, stopping polling");
			break;
		}
	...
	otc_info("Unified polling thread finished");
	return NULL;
}
...
OTC_PRIV int labjack_u12_start_polling_acquisition(const struct otc_dev_inst *sdi)
{
	...
	if (!devc->polling_thread) {
		otc_err("Failed to create polling thread");
		g_mutex_lock(&devc->polling_mutex);
		devc->polling_thread_running = FALSE;
		g_mutex_unlock(&devc->polling_mutex);
		return OTC_ERR;
	...
	otc_info("Unified polling acquisition started (interval=%u ms)", devc->poll_interval_ms);
...
OTC_PRIV int labjack_u12_stop_polling_acquisition(const struct otc_dev_inst *sdi)
{
	...
	if (!devc->polling_thread_running)
		return OTC_OK;  /* Already stopped */

	otc_info("Stopping unified polling acquisition...");
	...
	otc_info("Unified polling acquisition stopped");
