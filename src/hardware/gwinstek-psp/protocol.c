...
static void give_device_time_to_process(struct dev_context *devc)
{
	...
	if (sleeping_time > 0) {
		...
		otc_spew("Sleeping %" PRIi64 " us for processing", sleeping_time);
	}
}
    ...
    struct dev_context *devc)
{
	...
	otc_dbg("%d bytes left in buffer", bytes_left);

	if (bytes_left == 0) {
		/* 2, must already be set if we got here */
		otc_dbg("Device is in URPSP2 mode, terminator is CRLF");
	} else if (bytes_left == 1) {
		devc->msg_terminator_len = 3;
		otc_dbg("Device is in URPSP1 mode, terminator is CRCRLF");
	} else {
		otc_err("Don't know how to deal with %d bytes left", bytes_left);
		g_mutex_unlock(&devc->rw_mutex);
		return OTC_ERR;
	}
...
OTC_PRIV int gwinstek_psp_get_initial_voltage_target(struct dev_context *devc)
{
	if (!devc->output_enabled || (fabs(devc->current - devc->current_limit) >= 0.01)) {
		...
		otc_dbg("Set initial voltage target to %.2f", devc->voltage_target);
	} else {
		/* Would it be more correct to fail the scan here? */
		otc_warn("Could not determine actual voltage target, falling back to 0");
    ...
    struct dev_context *devc)
{
	...
	if (delta <= GWINSTEK_PSP_STATUS_POLL_TIME_MS * 1000) {
		otc_spew("Last status query was only %" PRIu64 "us ago, returning", delta);
		g_mutex_unlock(&devc->rw_mutex);
		return OTC_OK;
	}
	...
	if (ret < 0) {
		otc_err("Error %d reading from device.", ret);
		g_mutex_unlock(&devc->rw_mutex);
		return ret;
	}

	otc_dbg("Received: '%s'", buf);
	...
	if (ret != bytes_to_read || sscanf(buf, "V%fA%fW%fU%dI%f", &devc->voltage,
		...
		otc_err("Parsing status payload failed");
	...
	otc_spew("Status: voltage_or_0=%.2f, voltage_target=%.2f, current=%.3f, power=%.1f, "
...
OTC_PRIV int gwinstek_psp_receive_data(int fd, int revents, void *cb_data)
{
	...
	if (devc->set_voltage_target != devc->voltage_target &&
		 ...
		 * is more recent. */
		otc_dbg("Updating session voltage target to %.2f",
		       devc->voltage_target);
		otc_session_send_meta(sdi, OTC_CONF_VOLTAGE_TARGET,
