...
static struct otc_dev_driver gwinstek_psp_driver_info;
...
static const struct gwinstek_psp_model *model_lookup(const char *id_text)
{
	...
	otc_dbg("Could not find a matching model for: [%s].", id_text);
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	if (!serialcomm)
		serialcomm = "2400/8n1";
	if (!force_detect) {
		otc_err("The gwinstek-psp driver requires the force_detect parameter");
		return NULL;
	}
	...
	if (!model) {
		otc_err("Unsupported model ID '%s', aborting.", force_detect);
		return NULL;
	}
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	if (key != OTC_CONF_CONN && gwinstek_psp_get_all_values(sdi->conn, devc) < 0)
		return OTC_ERR;
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_VOLTAGE_TARGET:
		...
		if (devc->voltage_limit == voltage_limit) {
			otc_dbg("Correct limit (%dV) already set", voltage_limit);
		} else {
			otc_snprintf_ascii(msg, sizeof(msg), "SU %d\r\n", voltage_limit);
			if (gwinstek_psp_send_cmd(sdi->conn, devc, msg, TRUE) < 0)
				return OTC_ERR;
		}

		/* Set voltage output level */
		otc_snprintf_ascii(msg, sizeof(msg), "SV %05.2f\r\n", dval);
		if (gwinstek_psp_send_cmd(sdi->conn, devc, msg, TRUE) < 0)
			return OTC_ERR;
	...
	case OTC_CONF_CURRENT_LIMIT:
		...
		otc_snprintf_ascii(msg, sizeof(msg), "SI %04.2f\r\n", dval);
		if (gwinstek_psp_send_cmd(sdi->conn, devc, msg, TRUE) < 0)
			return OTC_ERR;
		break;
	case OTC_CONF_ENABLED:
...
OTC_REGISTER_DEV_DRIVER(gwinstek_psp_driver_info);
