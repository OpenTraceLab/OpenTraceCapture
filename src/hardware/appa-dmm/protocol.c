 ...
 */
static int appadmm_transform_display_data(const struct otc_dev_inst *arg_sdi,
	...
	const struct appadmm_response_data_read_display_s *arg_read_data)
{
	...
	if ((retr = otc_analog_init(&analog, &encoding, &meaning, &spec, 0))
		< OTC_OK)
		return retr;
	...
	case APPADMM_CHANNEL_INVALID:
		otc_err("Invalid channel selected when transforming readings");
		return OTC_ERR_BUG;
	...
	case APPADMM_CHANNEL_DISPLAY_SECONDARY:
		...
		if (!appadmm_is_wordcode(arg_display_data->reading)
			...
			case APPADMM_UNIT_PERCENT:
				...
				analog.meaning->mq = OTC_MQ_DIFFERENCE;
				break;
			...
			case APPADMM_FUNCTIONCODE_AC_V_PV_HFR:
				if (analog.meaning->unit == OTC_UNIT_AMPERE
					|| analog.meaning->unit == OTC_UNIT_VOLT
					|| analog.meaning->unit == OTC_UNIT_WATT) {
	...
	retr = otc_session_send(arg_sdi, &packet);
 ...
 * @retval OTC_ERR_... on error
 */
static int appadmm_transform_sample_id(const struct otc_dev_inst *arg_sdi,
	enum appadmm_channel_e arg_channel)
{
	struct otc_datafeed_packet packet;
 ...
 * @retval OTC_ERR_... on error
 */
static int appadmm_process_read_display(const struct otc_dev_inst *arg_sdi,
	const struct appadmm_response_data_read_display_s *arg_data)
{
	...
	packet.type = OTC_DF_FRAME_BEGIN;
	otc_session_send(arg_sdi, &packet);

	if ((retr = appadmm_transform_display_data(arg_sdi,
		APPADMM_CHANNEL_DISPLAY_PRIMARY,
		&arg_data->primary_display_data, arg_data)) < OTC_OK)
		return retr;

	if ((retr = appadmm_transform_display_data(arg_sdi,
		APPADMM_CHANNEL_DISPLAY_SECONDARY,
		&arg_data->secondary_display_data, arg_data)) < OTC_OK)
		return retr;

	packet.type = OTC_DF_FRAME_END;
	otc_session_send(arg_sdi, &packet);
 ...
 * @retval OTC_ERR_... on error
 */
static int appadmm_process_storage(const struct otc_dev_inst *arg_sdi,
	const struct appadmm_response_data_read_memory_s *arg_data)
{
	...
	for (xloop = 0;
		...
		if ((retr = appadmm_transform_display_data(arg_sdi,
			APPADMM_CHANNEL_DISPLAY_PRIMARY,
			&display_data[xloop], NULL)) < OTC_OK)
			return retr;

		if ((retr = appadmm_transform_sample_id(arg_sdi,
			APPADMM_CHANNEL_DISPLAY_SECONDARY)) < OTC_OK)
			return retr;
 ...
 * @retval OTC_ERR_... on error
 */
OTC_PRIV int appadmm_op_identify(const struct otc_dev_inst *arg_sdi)
{
	char *delim;
	char *model_name;
	char *serial_number;
 ...
 * @retval OTC_ERR_... on error
 */
OTC_PRIV int appadmm_op_storage_info(const struct otc_dev_inst *arg_sdi)
{
	struct appadmm_context *devc;
 ...
 */
OTC_PRIV int appadmm_acquire_live(int arg_fd, int arg_revents,
	void *arg_cb_data)
{
 ...
 * @retval OTC_ERR_... on error
 */
OTC_PRIV int appadmm_acquire_storage(int arg_fd, int arg_revents,
	void *arg_cb_data)
{
	struct otc_dev_inst *sdi;
	struct appadmm_context *devc;
 ...
 * @retval OTC_OK on success
 * @retval OTC_ERR_... on failure
 */
OTC_PRIV int appadmm_clear_context(struct appadmm_context *arg_devc)
{
	if (arg_devc == NULL)
		return OTC_ERR_BUG;
	...
	otc_sw_limits_init(&arg_devc->limits);
 ...
 * @retval OTC_ERR_... on error
 */
OTC_PRIV int appadmm_clear_storage_info(struct appadmm_storage_info_s *arg_storage_info)
{
	int xloop;
