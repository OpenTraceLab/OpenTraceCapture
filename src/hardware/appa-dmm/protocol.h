...
#ifndef LIBOPENTRACECAPTURE_HARDWARE_APPA_DMM_PROTOCOL_H
#define LIBOPENTRACECAPTURE_HARDWARE_APPA_DMM_PROTOCOL_H
...
struct appadmm_response_data_read_information_s {
	...
	char otc_serial_number[17]; /**< String 0x20 filled serial number of device */
...
struct appadmm_response_data_read_memory_s {
	uint8_t data[OTC_TP_APPA_MAX_DATA_SIZE]; /**< Requested data */
...
struct appadmm_context {
	struct otc_tp_appa_inst appa_inst; /**< APPA transport protocol instance */
	...
	struct otc_sw_limits limits; /**< Limits for data acquisition */
...
/* ****** Resolvers / Tables ****** */
OTC_PRIV const char *appadmm_channel_name(const enum appadmm_channel_e arg_channel);
OTC_PRIV const char *appadmm_model_id_name(const enum appadmm_model_id_e arg_model_id);
