 ...
 */
static int siglent_sds_event_wait(const struct otc_dev_inst *sdi)
{
	char *buf;
	long s;
	int out;
	...
	if (devc->wait_status == 1) {
		do {
			...
			if (otc_scpi_get_string(sdi->conn, ":INR?", &buf) != OTC_OK)
				return OTC_ERR;
			otc_atoi(buf, &out);
			g_free(buf);
			g_usleep(s);
		...
		if ((devc->timebase < 0.51) && (devc->timebase > 0.99e-6)) {
			...
			otc_spew("Sleeping for %ld usecs after trigger, "
...
static int siglent_sds_trigger_wait(const struct otc_dev_inst *sdi)
{
	struct dev_context *devc;
...
static int siglent_sds_stop_wait(const struct otc_dev_inst *sdi)
{
	return siglent_sds_event_wait(sdi);
}

/* Send a configuration setting. */
OTC_PRIV int siglent_sds_config_set(const struct otc_dev_inst *sdi, const char *format, ...)
{
	va_list args;
	int ret;
...
OTC_PRIV int siglent_sds_capture_start(const struct otc_dev_inst *sdi)
{
	...
	case SPO_MODEL:
		if (devc->data_source == DATA_SOURCE_SCREEN) {
			...
			otc_dbg("Starting data capture for active frameset %" PRIu64 " of %" PRIu64,
				devc->num_frames + 1, devc->limit_frames);
			if (siglent_sds_config_set(sdi, "ARM") != OTC_OK)
				return OTC_ERR;
			if (otc_scpi_get_string(sdi->conn, ":INR?", &buf) != OTC_OK)
				return OTC_ERR;
			otc_atoi(buf, &out);
			...
			if (out == DEVICE_STATE_TRIG_RDY) {
				...
				otc_spew("Device triggered.");
				siglent_sds_set_wait_event(devc, WAIT_BLOCK);
				return OTC_OK;
			} else {
				otc_spew("Device did not enter ARM mode.");
				return OTC_ERR;
			}
			...
			otc_dbg("Starting data capture for history frameset.");
			if (siglent_sds_config_set(sdi, "FPAR?") != OTC_OK)
				return OTC_ERR;
			ret = otc_scpi_read_data(sdi->conn, buf, 200);
			if (ret < 0) {
			...
			if (devc->limit_frames > framecount)
				otc_err("Frame limit higher than frames in buffer of device!");
			else if (devc->limit_frames == 0)
				devc->limit_frames = framecount;
			otc_dbg("Starting data capture for history frameset %" PRIu64 " of %" PRIu64,
				devc->num_frames + 1, devc->limit_frames);
			if (siglent_sds_config_set(sdi, "FRAM %i", devc->num_frames + 1) != OTC_OK)
				return OTC_ERR;
			if (siglent_sds_channel_start(sdi) != OTC_OK)
				return OTC_ERR;
...
OTC_PRIV int siglent_sds_channel_start(const struct otc_dev_inst *sdi)
{
	...
	otc_dbg("Start reading data from channel %s.", ch->name);

	switch (devc->model->series->protocol) {
	case NON_SPO_MODEL:
	case SPO_MODEL:
		s = (ch->type == OTC_CHANNEL_LOGIC) ? "D%d:WF?" : "C%d:WF? ALL";
		if (otc_scpi_send(sdi->conn, s, ch->index + 1) != OTC_OK)
			return OTC_ERR;
		siglent_sds_set_wait_event(devc, WAIT_NONE);
		break;
	case ESERIES:
		if (ch->type == OTC_CHANNEL_ANALOG) {
			if (otc_scpi_send(sdi->conn, "C%d:WF? ALL",
				ch->index + 1) != OTC_OK)
				return OTC_ERR;
		}
		siglent_sds_set_wait_event(devc, WAIT_NONE);
...
static int siglent_sds_read_header(struct otc_dev_inst *sdi)
{
	...
	ret = otc_scpi_read_data(scpi, buf, SIGLENT_HEADER_SIZE);
	if (ret < SIGLENT_HEADER_SIZE) {
		otc_err("Read error while reading data header.");
		return OTC_ERR;
	}
	otc_dbg("Device returned %i bytes.", ret);
...
static int siglent_sds_get_digital(const struct otc_dev_inst *sdi, struct otc_channel *ch)
{
	struct otc_scpi_dev_inst *scpi = sdi->conn;
	struct dev_context *devc = sdi->priv;
	...
	for (l = sdi->channels; l; l = l->next) {
		...
		if (!ch->enabled)
			continue;
		if (otc_scpi_send(sdi->conn, "D%d:WF? DAT2", ch->index) != OTC_OK)
			return OTC_ERR;
		if (otc_scpi_read_begin(scpi) != OTC_OK)
			return TRUE;
		len = otc_scpi_read_data(scpi, buf, -1);
		if (len < 0)
			return TRUE;
		len -= 15;
	...
	/* Combining the lower and higher channel buffers into one buffer for opentracelab. */
...
OTC_PRIV int siglent_sds_receive(int fd, int revents, void *cb_data)
{
	...
	case WAIT_NONE:
		break;
	case WAIT_TRIGGER:
		if (siglent_sds_trigger_wait(sdi) != OTC_OK)
			return TRUE;
		if (siglent_sds_channel_start(sdi) != OTC_OK)
			return TRUE;
		return TRUE;
	case WAIT_BLOCK:
		...
		break;
	case WAIT_STOP:
		if (siglent_sds_stop_wait(sdi) != OTC_OK)
			return TRUE;
		if (siglent_sds_channel_start(sdi) != OTC_OK)
			return TRUE;
		return TRUE;
	...
	if (ch->type == OTC_CHANNEL_ANALOG) {
		if (devc->num_block_bytes == 0) {
			...
			case SPO_MODEL:
				...
				otc_dbg("Waiting %.f0 ms for device to prepare the output buffers", wait / 1000);
				g_usleep(wait);
				if (otc_scpi_read_begin(scpi) != OTC_OK)
					return TRUE;
				break;
			...
			otc_dbg("New block with header expected.");
			len = siglent_sds_read_header(sdi);
			if (len == 0)
				/* Still reading the header. */
				return TRUE;
			if (len == -1) {
				otc_err("Read error, aborting capture.");
			...
			do {
				...
				if (devc->num_block_bytes > devc->num_samples) {
					...
					otc_dbg("Requesting: %" PRIu64 " bytes.", devc->num_samples - devc->num_block_bytes);
					len = otc_scpi_read_data(scpi, (char *)devc->buffer, devc->num_samples-devc->num_block_bytes);
					if (len == -1) {
						otc_err("Read error, aborting capture.");
						std_session_send_df_frame_end(sdi);
						sdi->driver->dev_acquisition_stop(sdi);
				...
				otc_dbg("Received block: %i, %d bytes.", devc->num_block_read, len);
				if (ch->type == OTC_CHANNEL_ANALOG) {
				...
				if (devc->num_samples == (devc->num_block_bytes - SIGLENT_HEADER_SIZE)) {
					otc_dbg("Transfer has been completed.");
					devc->num_header_bytes = 0;
					devc->num_block_bytes = 0;
					read_complete = TRUE;
					if (!otc_scpi_read_complete(scpi)) {
...
OTC_PRIV int siglent_sds_get_dev_cfg(const struct otc_dev_inst *sdi)
{
	...
	if (devc->model->has_digital) {
		...
		otc_dbg("Check logic analyzer channel state.");
		devc->la_enabled = FALSE;
		cmd = g_strdup_printf("DI:SW?");
		res = otc_scpi_get_bool(sdi->conn, cmd, &status);
		g_free(cmd);
		if (res != OTC_OK)
			return OTC_ERR;
		otc_dbg("Logic analyzer status: %s", status ? "On" : "Off");
		if (status) {
	...
	/* Timebase. */
	if (otc_scpi_get_float(sdi->conn, ":TDIV?", &devc->timebase) != OTC_OK)
		return OTC_ERR;
	...
	for (i = 0; i < devc->model->analog_channels; i++) {
		...
		res = otc_scpi_get_float(sdi->conn, cmd, &devc->attenuation[i]);
		g_free(cmd);
		if (res != OTC_OK)
			return OTC_ERR;
	}
	...
	if (siglent_sds_get_dev_cfg_vertical(sdi) != OTC_OK)
		return OTC_ERR;
	...
	response = NULL;
	tokens = NULL;
	if (otc_scpi_get_string(sdi->conn, "TRSE?", &response) != OTC_OK)
		return OTC_ERR;
	tokens = g_strsplit(response, ",", 0);
	num_tokens = g_strv_length(tokens);
	if (num_tokens < 4) {
		otc_dbg("IDN response not according to spec: %80.s.", response);
		g_strfreev(tokens);
		g_free(response);
		return OTC_ERR_DATA;
	}
	...
	response = "";
	trigger_pos = 0;
	// if (otc_scpi_get_string(sdi->conn, g_strdup_printf("%s:TRDL?", devc->trigger_source), &response) != OTC_OK)
	// 	return OTC_ERR;
	// len = strlen(response);
	len = strlen(tokens[4]);
	if (!g_ascii_strcasecmp(tokens[4] + (len - 2), "us")) {
		trigger_pos = atof(tokens[4]) / OTC_GHZ(1);
		otc_dbg("Current trigger position us %s.", tokens[4] );
	} else if (!g_ascii_strcasecmp(tokens[4] + (len - 2), "ns")) {
		trigger_pos = atof(tokens[4]) / OTC_MHZ(1);
		otc_dbg("Current trigger position ms %s.", tokens[4] );
	} else if (!g_ascii_strcasecmp(tokens[4] + (len - 2), "ms")) {
		trigger_pos = atof(tokens[4]) / OTC_KHZ(1);
		otc_dbg("Current trigger position ns %s.", tokens[4] );
	} else if (!g_ascii_strcasecmp(tokens[4] + (len - 2), "s")) {
		trigger_pos = atof(tokens[4]);
		otc_dbg("Current trigger position s %s.", tokens[4] );
	};
	devc->horiz_triggerpos = trigger_pos;

	otc_dbg("Current horizontal trigger position %.10f.", devc->horiz_triggerpos);

	/* Trigger slope. */
	cmd = g_strdup_printf("%s:TRSL?", devc->trigger_source);
	g_free(devc->trigger_slope);
	devc->trigger_slope = NULL;
	res = otc_scpi_get_string(sdi->conn, cmd, &devc->trigger_slope);
	g_free(cmd);
	if (res != OTC_OK)
		return OTC_ERR;
	...
	if (g_str_has_prefix(tokens[2], "C")) {
		...
		res = otc_scpi_get_float(sdi->conn, cmd, &devc->trigger_level);
		g_free(cmd);
		if (res != OTC_OK)
			return OTC_ERR;
...
OTC_PRIV int siglent_sds_get_dev_cfg_horizontal(const struct otc_dev_inst *sdi)
{
	...
	case NON_SPO_MODEL:
		...
		res = otc_scpi_get_string(sdi->conn, cmd, &sample_points_string);
		g_free(cmd);
		samplerate_scope = 0;
		fvalue = 0;
		if (res != OTC_OK) {
		...
		if (g_strstr_len(sample_points_string, -1, "Mpts") != NULL) {
			...
			if (otc_atof_ascii(sample_points_string, &fvalue) != OTC_OK) {
				otc_dbg("Invalid float converted from scope response.");
				g_free(sample_points_string);
				return OTC_ERR;
			}
	...
	case ESERIES:
		cmd = g_strdup_printf("SANU? C1");
		if (otc_scpi_get_float(sdi->conn, cmd, &fvalue) != OTC_OK)
			return OTC_ERR;
	...
	otc_dbg("Current samplerate: %0f.", devc->samplerate);
	otc_dbg("Current memory depth: %" PRIu64 ".", devc->memory_depth_analog);
