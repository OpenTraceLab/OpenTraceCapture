...
static const uint32_t devopts[] = {
	...
	OTC_CONF_SAMPLE_INTERVAL | OTC_CONF_GET | OTC_CONF_SET | OTC_CONF_LIST,
	OTC_CONF_DATALOG | OTC_CONF_GET,
...
OTC_PRIV const uint64_t kecheng_kc_330b_sample_intervals[][2] = {
	{ 1, 8 },
	{ 1, 2 },
	{ 1, 1 },
	{ 2, 1 },
...
static int scan_kecheng(struct otc_dev_driver *di,
		struct otc_usb_dev_inst *usb, char **model)
{
	...
	if (ret != 0) {
		...
		otc_dbg("Failed to send Identify command: %s", libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	if (ret != 0) {
		libusb_close(usb->devhdl);
		otc_dbg("Failed to receive response: %s", libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	if (len < 2 || buf[0] != (CMD_IDENTIFY | 0x80) || buf[1] > 30) {
		otc_dbg("Invalid response to Identify command");
		return OTC_ERR;
	}
...
static GSList *scan(struct otc_dev_driver *di, GSList *options)
{
	...
	devices = NULL;
	if ((usb_devices = otc_usb_find(drvc->otc_ctx->libusb_ctx, USB_CONN))) {
		/* We have a list of otc_usb_dev_inst matching the connection
		 * string. Wrap them in otc_dev_inst and we're done. */
		for (l = usb_devices; l; l = l->next) {
			if (scan_kecheng(di, l->data, &model) != OTC_OK)
				continue;
...
static int dev_open(struct otc_dev_inst *sdi)
{
	...
	if ((ret = libusb_set_configuration(usb->devhdl, 1))) {
		otc_err("Failed to set configuration: %s.", libusb_error_name(ret));
		return OTC_ERR;
	}
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_SPL_WEIGHT_FREQ:
		if (devc->mqflags & OTC_MQFLAG_SPL_FREQ_WEIGHT_A)
			*data = g_variant_new_string("A");
		else
			*data = g_variant_new_string("C");
		break;
	case OTC_CONF_SPL_WEIGHT_TIME:
		if (devc->mqflags & OTC_MQFLAG_SPL_TIME_WEIGHT_F)
	...
	const struct otc_dev_inst *sdi, const struct otc_channel_group *cg)
{
	...
	case OTC_CONF_SPL_WEIGHT_FREQ:
		if ((idx = std_str_idx(data, ARRAY_AND_SIZE(weight_freq))) < 0)
			return OTC_ERR_ARG;
		devc->mqflags &= ~(OTC_MQFLAG_SPL_FREQ_WEIGHT_A | OTC_MQFLAG_SPL_FREQ_WEIGHT_C);
		devc->mqflags |= (weight_freq[idx][0] == 'A') ? OTC_MQFLAG_SPL_FREQ_WEIGHT_A : OTC_MQFLAG_SPL_FREQ_WEIGHT_C;
		devc->config_dirty = TRUE;
		break;
	case OTC_CONF_SPL_WEIGHT_TIME:
		if ((idx = std_str_idx(data, ARRAY_AND_SIZE(weight_time))) < 0)
			return OTC_ERR_ARG;
		devc->mqflags &= ~(OTC_MQFLAG_SPL_TIME_WEIGHT_F | OTC_MQFLAG_SPL_TIME_WEIGHT_S);
		devc->mqflags |= (weight_time[idx][0] == 'F') ? OTC_MQFLAG_SPL_TIME_WEIGHT_F : OTC_MQFLAG_SPL_TIME_WEIGHT_S;
		devc->config_dirty = TRUE;
		break;
	case OTC_CONF_DATA_SOURCE:
...
static int dev_acquisition_start(const struct otc_dev_inst *sdi)
{
	...
	if (devc->data_source == DATA_SOURCE_LIVE) {
		...
		if (kecheng_kc_330b_status_get(sdi, &ret) != OTC_OK)
			return OTC_ERR;
		if (ret != DEVICE_ACTIVE) {
			otc_err("Device is inactive");
		...
		if (kecheng_kc_330b_log_info_get(sdi, buf) != OTC_OK)
			return OTC_ERR;
		devc->mqflags = buf[4] ? OTC_MQFLAG_SPL_TIME_WEIGHT_S : OTC_MQFLAG_SPL_TIME_WEIGHT_F;
		devc->mqflags |= buf[5] ? OTC_MQFLAG_SPL_FREQ_WEIGHT_C : OTC_MQFLAG_SPL_FREQ_WEIGHT_A;
		...
		src = otc_config_new(OTC_CONF_SAMPLE_INTERVAL, gvar);
	...
	usb_source_add(sdi->session, drvc->otc_ctx, 10,
	...
	if (ret != 0 || len != 1) {
		otc_dbg("Failed to start acquisition: %s", libusb_error_name(ret));
...
static struct otc_dev_driver kecheng_kc_330b_driver_info = {
...
OTC_REGISTER_DEV_DRIVER(kecheng_kc_330b_driver_info);
